...
from gerar_ns import enriquecer_trechos, calc_manning, to_ll, gerar_geojson
...
def gerar_ns_satelite(ns_id, trecho, pvs, nucleo, out_path):
    """PDF Satélite - Imagem Esri + perfil"""
    pvi_n, pvf_n = trecho["pv_ini"], trecho["pv_fim"]
    pvi, pvf = pvs.get(pvi_n, {}), pvs.get(pvf_n, {})
    ext = trecho.get("ext_m", 1)
    ...
    ax.set_title(f"PLANTA SATÉLITE - NS {ns_id:03d} · {nucleo}", fontsize=12, fontweight="bold")
    ax.axis("off")
...
def processar_arquivo(dwg_path, nucleo):
    ...
    for i, t in enumerate(trechos):
        ...
        try:
            ...
            # 3. Satélite
            gerar_ns_satelite(
                ns_id, t, pvs, nucleo, ns_dir / f"Satelite NS {ns_id:03d} {pvi} ATE {pvf}.pdf"
            )

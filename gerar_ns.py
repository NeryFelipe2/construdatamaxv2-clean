...
Saidas por trecho:
  ...
  03_DESENHOS/         -> PDF A3 planta(cartografia) + perfil
...
def adicionar_base_cartografica(ax, crs="EPSG:31983", zoom=18):
    """Adiciona base cartografica, nunca imagem de satelite."""
    try:
    ...
    providers = []
    try:
        providers.append(cx.providers.CartoDB.Positron)
    except Exception:
        pass
    try:
        providers.append(cx.providers.OpenStreetMap.Mapnik)
    except Exception:
        pass
    for source in providers:
        for z in (zoom, max(1, zoom - 1)):
            try:
                cx.add_basemap(ax, crs=crs, source=source, zoom=z)
                return True
            except Exception:
                continue
    return False
...
def gerar_ns_sat(ns_id, trecho, pvs, nucleo, out_path):
    """Gera PDF A3 com cartografia + rede projetada + perfil longitudinal."""
    ...
    if not adicionar_base_cartografica(ax_planta, zoom=18):
        ax_planta.set_facecolor("#f8fafc")
        ax_planta.grid(True, color="#cbd5e1", alpha=0.65)
        ax_planta.text(0.5, 0.04, "Base cartografica indisponivel - coordenadas UTM",
                       transform=ax_planta.transAxes, ha="center", fontsize=8, color="#475569")
    ax_planta.plot([p0["x"], p1["x"]], [p0["y"], p1["y"]],
                   color="red", linewidth=3.2, zorder=10, label="Rede projetada")
    for pv_nome, pv in [(trecho["pv_ini"], p0), (trecho["pv_fim"], p1)]:
        ax_planta.plot(pv["x"], pv["y"], 'ro', markersize=8, zorder=11)
    ...
    ax_planta.legend(loc="lower right", fontsize=8)
    ...
    for i, t in enumerate(trechos):
        ...
        try:
            ...
            try:
                ...
                pass  # cartografia pode falhar sem internet

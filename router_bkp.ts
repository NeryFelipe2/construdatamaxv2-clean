...
// <workflow-map>
// Workflow : GestÃƒÂ£o WhatsApp Ã¢â‚¬â€ Router Central
...
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// ReceberEvolutionApi
//    â†’ ParseEventoWhatsapp
//      â†’ Ignorar
//        â†’ EncaminharParaSubWorkflow
// </workflow-map>
...
@workflow({
    id: 'CJRFUtzbL3pGpb4s',
    name: 'GestÃƒÂ£o WhatsApp Ã¢â‚¬â€ Router Central',
    active: true,
    settings: {
        executionOrder: 'v1',
...
export class GestaOWhatsappARouterCentralWorkflow {
    // =====================================================================
...
if (isFromMe) {
  const isComando = /^(@|menu|ajuda|comandos|status|equipe|projetos|dashboard|ia|ai|rdo|[1-9sm])/i.test(trimmed);
...
// ========== PERGUNTAS DE SUPERVISÃƒÆ’O PARA DIRETORES ==========
...
const RDO_DIRETOR_SUPERVISAO = [
  ...
  { num: 2, label: 'Principais decisÃƒÂµes tomadas', tag: 'decisoes' },
  { num: 3, label: 'Riscos ou alertas identificados', tag: 'riscos' },
  { num: 4, label: 'PrevisÃƒÂ£o de entrega prÃƒÂ³x. marco', tag: 'previsao_marco' },
  { num: 5, label: 'ObservaÃƒÂ§ÃƒÂµes gerais', tag: 'observacoes' },
];

// ========== MENU INTERATIVO ==========
// Detecta saudaÃƒÂ§ÃƒÂ£o, ajuda, ou comando @
const isSaudacao = /^(oi|ola|olÃƒÂ¡|bom dia|boa tarde|boa noite|opa|menu|opÃƒÂ§ÃƒÂµes|opcoes|inicio|inÃƒÂ­cio|comeÃƒÂ§ar|comecar|start)$/i.test(trimmed);
...
function montarMenu(p, phoneDetectado) {
  if (!p) return 'Ã°Å¸Â¤â€“ ConstruDataMax\\n\\nNÃƒÂ£o consegui identificar seu projeto. Telefone detectado: *' + phoneDetectado + '*\\nFala com o admin para te cadastrar.';

  // Menu de gestor/diretor
  if (p.isGestor) {
    const menuLinhas = [
      'Ã°Å¸Â¤â€“ *ConstruDataMax GestÃƒÂ£o 360*',
      'OlÃƒÂ¡ *' + p.responsavel + '*!',
      '',
      'Ã°Å¸â€œÅ  *OpÃƒÂ§ÃƒÂµes de Comando:*',
      '1Ã¯Â¸ÂÃ¢Æ’Â£ *Status RDO Hoje*',
      '2Ã¯Â¸ÂÃ¢Æ’Â£ *Equipe e Contatos*',
      '3Ã¯Â¸ÂÃ¢Æ’Â£ *Projetos Ativos*',
      '4Ã¯Â¸ÂÃ¢Æ’Â£ *Dashboard Consolidado*',
      '5Ã¯Â¸ÂÃ¢Æ’Â£ *Reenviar CobranÃƒÂ§a* (Alerta Geral)',
      '6Ã¯Â¸ÂÃ¢Æ’Â£ *Falar com IA* (sÃƒÂ³ Felipe)',
      '7Ã¯Â¸ÂÃ¢Æ’Â£ *Cobrar RDO* (Dispara formulÃƒÂ¡rio)',
      '8Ã¯Â¸ÂÃ¢Æ’Â£ *Meu RDO Diretor* (SupervisÃƒÂ£o)',
      '',
      'Ã°Å¸â€œÅ’ *Comandos extras:*',
      'Ã¢â‚¬Â¢ @rdo <projeto|todos> Ã¢â‚¬â€ Dispara RDO pros engenheiros',
      'Ã¢â‚¬Â¢ @tarefa <nome|todos> <desc> Ã¢â‚¬â€ Delegar tarefa',
      'Ã¢â‚¬Â¢ @avisar <projeto> <msg> Ã¢â‚¬â€ Broadcast',
      'Ã¢â‚¬Â¢ @meurdo Ã¢â‚¬â€ Preencher RDO de supervisÃƒÂ£o',
      '',
      '_Ã¢â€“Â¶Ã¯Â¸Â Digite o nÃƒÂºmero ou use @comandos._',
      'Ã°Å¸â€â€” https://construdatamaxv2-clean.vercel.app'
    ];
    return menuLinhas.join('\\n');
  }

  const tituloForm = p.isSalaTecnica ? 'Ã°Å¸â€œÂ *Atividades do Dia Ã¢â‚¬â€ preencha:*' : 'Ã°Å¸â€œÂ *Para responder o RDO de hoje:*';
  const linhas = [
    'Ã°Å¸Â¤â€“ *ConstruDataMax Ã¢â‚¬â€ OperaÃƒÂ§ÃƒÂ£o*',
    'OlÃƒÂ¡ *' + p.responsavel + '*! Projeto: *' + p.nome + '*',
    '',
    tituloForm,
    ''
  ];
  ...
  linhas.push('');
  linhas.push('Ã°Å¸â€™Â¬ *Como preencher (jeito rÃƒÂ¡pido):*');
  linhas.push('Copia o bloco abaixo, edita os valores e manda de volta numa mensagem sÃƒÂ³:');
  linhas.push('');
  for (const q of p.perguntas) {
    linhas.push(q.num + ': ');
  }
  linhas.push('');
  linhas.push('_Pode mandar tudo de uma vez ou um por linha. TambÃƒÂ©m aceita texto livre nas pendÃƒÂªncias._');
  linhas.push('');
  linhas.push('Ã°Å¸â€ Ëœ *Outras OpÃƒÂ§ÃƒÂµes:*');
  linhas.push('*( M )* Ã¢â‚¬â€ Menu e Ajuda');
  linhas.push('*( S )* Ã¢â‚¬â€ Status do RDO');
  return linhas.join('\\n');
}
...
const shortcutMatch = trimmed.match(/^([1-8sm])$/i);

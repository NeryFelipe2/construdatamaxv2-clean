...
def _extrair_tubos(gdf):
    ...
    for layer in layers:
        ...
        inclui = any(p in layer_upper for p in [
            ...
            "PONTOS_IDENTIFICACAO"  # ProSaneamento padrão: PS_PONTOS_IDENTIFICACAO_LIN
        ])
        ...
        # USA REGEX PARA EVITAR FALSO POSITIVO: "PONTOS" não pode estar em "PONTOS_IDENTIFICACAO"
        import re
        exclui = bool(
            re.search(r'(PERFIL|DETALHE|CORTE|BIFILAR|\bTXT\b|TEXTO|COTA|DIMENSÃO|HACHURA|MOBILIÁRIO|\bRUAS\b|\bQUADRAS\b|\bPONTOS\b|\bCAIXAS\b)', layer_upper)
        )
...
def ler_dxf_teteu(dxf_path, modo="hibrido"):
    ...
    # ── 1. CARREGAR DXF ──────────────────────────────────────────────────────
    try:
        # Tenta diferentes nomes de layer (compatibilidade com ProSaneamento)
        gdf = None
        for lyr in ["entities", "*Model_Space", "0", None]:
            try:
                gdf = gpd.read_file(dxf_path, layer=lyr) if lyr else gpd.read_file(dxf_path)
                if gdf is not None and len(gdf) > 0:
                    _log(f"  Lido do layer: {lyr}", "INFO")
                    break
            except Exception:
                continue
        if gdf is None or len(gdf) == 0:
            raise ValueError("Nenhuma entidade lida")

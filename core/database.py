...
def _ensure_sqlite_columns() -> None:
    ...
    desired_columns = {
        ...
        "rdo": {
            ...
            "empreiteira": "TEXT",
            "encarregado": "TEXT",
            "origem": "TEXT DEFAULT 'digital'",
            "status_revisao": "TEXT DEFAULT 'rascunho'",
            "assinatura_presente": "BOOLEAN DEFAULT 0",
            "deleted_at": "DATETIME",
        },
    }
...
def dashboard_metricas(nucleo: str = "") -> dict[str, Any]:
    if nucleo:
        ...
        with get_session() as session:
            rdos = session.execute(select(RDO).where(RDO.nucleo == nucleo, RDO.deleted_at.is_(None))).scalars().all()
    else:
        ...
        with get_session() as session:
            rdos = session.execute(select(RDO).where(RDO.deleted_at.is_(None))).scalars().all()

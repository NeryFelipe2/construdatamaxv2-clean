...
6 arquivos por pasta: Desenho, Satelite, A4, JSON, OSE, Mapa HTML
"""
...
from gerar_ns import gerar_ns_desenho, gerar_ns_sat
...
def gerar_satelite_completo(ns_id, t, pvs, nucleo, out_path):
    """PDF Satelite: imagem Esri + perfil longitudinal."""
    p0 = pvs.get(t["pv_ini"], {}); p1 = pvs.get(t["pv_fim"], {})
    if not p0.get("x") or not p1.get("x"):
        return
    pvi = t["pv_ini"]; pvf = t["pv_fim"]; ext = t["ext_m"]
    ...
    try:
        ...
        cx.add_basemap(ax_s, crs="EPSG:31983", source=cx.providers.Esri.WorldImagery, zoom=18)
    except Exception:
    ...
    for tipo, path, nucleo in NUCLEOS:
        ...
        for i, t in enumerate(trechos):
            ...
            try:
                ...
                # 2. Satelite (layout Israel)
                try:
                    gerar_satelite_completo(ns_id, t, pvs, nucleo,
                        ns_dir / f"Satelite NS {ns_id:03d} {pvi} ATE {pvf}.pdf")
                except Exception:
                    pass

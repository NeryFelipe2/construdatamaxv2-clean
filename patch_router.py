...
# 1. Adicionar contatos de engenharia
text = text.replace(
...
# 3. Atualizar Mateus em Osasco com custos
text = text.replace(
...
# 4. Atualizar Sala Técnica com todos os envolvidos
...
# 5. Atualizar diretorias
old_dirs = """  if (p.includes('999425397')) return {
...
new_dirs = """  if (p.includes('999425397')) return {
    nome: 'Diretoria Pardinho/Osasco/RK', responsavel: 'Luiz Fernando', isGestor: true, isDiretor: true,
...
new_menu = """      '7️⃣ *Cobrar RDO* (Dispara formulário)',
      '8️⃣ *Meu RDO Diretor* (Supervisão)',
      '9️⃣ *Lembrar Tarefas* (Cobra os diretores)',
      '🔟 *Criar Tarefas* (Como criar)',
      '1️⃣1️⃣ *Plano de Custos* (Informações de custo)',
      '',
      '📌 *Comandos de IA e Plataforma:*',
      '• @gerar dashboard',
      '• @gerar machine learning',
      '• @gerar relatorios',
      '• @subir para a gestão 360',
      '📌 *Comandos extras:*',"""
text = text.replace(old_menu, new_menu)

# 8. Converter atalhos numericos ate 11
...
# 9. Novos comandos 'lembrar', 'criartarefa', 'planocustos', 'gerar', 'subir'
cmds_logic = """  } else if (cmd === 'meurdo' || cmd === 'meurdo') {"""
...
text = text.replace(cmds_logic, new_cmds_logic)

# 10. Corrigir Tarefa (quem pode ser focado e garantir Fabi só sala tecnica)
...
new_task = """      const TODOS_ENG = [
      ...
      if (alvo === 'todos') {
        ...
        if(tgt.length > 0) {
          // Bloqueio explícito Fabrizzio (só manda pra sala_tecnica)
...
# Ajustar o RDO envio de perguntas para incluir rk e todos novos de sala tecnica, bloqueando Fabrizzio
...
# Ajustar Fabrizzio restrição no if do PARdinha / Osasco (Fabrizzio não envia nem recebe)
text = text.replace(

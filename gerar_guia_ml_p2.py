...
GANTT_ROWS = [
    ...
    ("Morro do Tetéu","DGS",4,[(0,0,16,"2E75B6")]),
    ("Pantanal Baixo","DGS",4,[(0,0,16,"2E75B6")]),
    ("Vila dos Criadores","DGS",3,[(0,0,16,"2E75B6")]),
    ("São Manoel","DGS",3,[(0,0,6,"2E75B6")]),
    ("João Carlos","DGS",3,[(0,0,2,"2E75B6")]),
    ("Vila Israel","DGS",4,[(0,0,6,"2E75B6")]),
    ("","","",None),
    ("═══ NOVOS GRUPOS ═══","","",None),
    ("R3 Cruzeiro/Prog II  ▲Mob 07/Abr","DGS",3,[(0,1,1,"FFC000"),(0,2,14,"70AD47")]),
    ("R8 Noroeste/Gilda  ▲Mob 07/Abr","DGS",3,[(0,1,1,"FFC000"),(0,2,14,"70AD47")]),
    ("R7 Piratininga/NW  ▲Mob 22/Abr","DGS",3,[(0,3,1,"FFC000"),(1,0,13,"70AD47")]),
    ("R4 Cruzeiro/Penha  ▲Mob 22/Abr","DGS",3,[(0,3,1,"FFC000"),(1,0,13,"70AD47")]),
    ("R2 Cruzeiro/Prog I  ▲Mob 12/Mai","DGS",2,[(1,1,1,"FFC000"),(1,2,12,"92D050")]),
    ("R5 Cruzeiro/Bento  ▲Mob 12/Mai","DGS",2,[(1,1,1,"FFC000"),(1,2,12,"92D050")]),
    ("R1 Zona Leste  ▲Mob 01/Jun","DGS",3,[(2,0,1,"FFC000"),(2,1,11,"70AD47")]),
    ("R6 Monte Serrat  ▲Mob 01/Jun","DGS",4,[(2,0,1,"FFC000"),(2,1,11,"2E75B6")]),
    ("","","",None),
    ...
    ("Cadastro Comercial — todos","SABESP+DGS","-",[(0,0,18,"FFD966")]),
    ...
    ("Boletim Medição Mensal (dia 20)","DGS","-",[(0,3,1,"C9C9C9"),(1,3,1,"C9C9C9"),
...
CHECKLIST = [
    ...
    ("01","PROJETO",    "Projeto executivo aprovado pela SABESP",         "DGS Eng.",  "30 dias", "Plantas aprovadas c/ carimbo SABESP"),
    ...
    ("04","LICENÇAS",   "Licença de sinalização (Prefeitura Santos)",      "Adm. DGS",  "15 dias", "Alvará sinalização"),
    ("05","LICENÇAS",   "Comunicação à SABESP (Nota de Início)",          "DGS Eng.",  "10 dias", "E-mail protocolo"),
    ...
    ("11","SOCIAL",     "Reunião informativa com moradores",              "DGS+SABESP","10 dias", "Ata de reunião"),
    ("12","COMERCIAL",  "Cadastro Comercial SABESP: confirmar dados",     "SABESP+DGS","10 dias", "Planilha CC atualizada"),
    ...
    ("18","CANTEIRO",   "Placa de obra instalada (padrão SABESP)",       "DGS",       "3 dias",   "Foto placa"),
    ...
    ("21","LAVAGEM",    "Lavagem SABESP AGENDADA (mín 2 dias antes)",     "DGS+SABESP","Durante", "Protocolo agendamento"),
    ("22","MEDIÇÃO",    "Planilha de medição diária configurada",         "DGS",       "1 dia",   "Execução_Geral.xlsx núcleo"),
    ("23","FISCAL",     "Fiscal SABESP comunicado sobre início",          "DGS",       "3 dias",  "E-mail fiscal"),
...
LAVAGEM_STEPS = [
    ("1","Concluir 100% assentamento da rede no trecho","Após último tubo","—","DGS","—","Diário de obra assinado"),
    ("2","Registrar comprimento e diâmetro do trecho","Após conclusão","2h","DGS + Topógrafo","—","Caderneta de campo"),
    ("3","Calcular volume de lavagem (L = π×(D/2)²×L×2)","Antes agendamento","30min","Eng. Responsável","—","Planilha de cálculo"),
    ("4","⚠ LIGAR para SABESP e agendar lavagem","MÍNIMO 2 DIAS ANTES","—","DGS (responsável)","48h mín.","Protocolo número: ________"),
    ("5","Preencher formulário de agendamento SABESP","Ao agendar","30min","DGS","48h mín.","Form. SABESP LA-001"),
    ("6","Confirmar agendamento 1 dia antes (e-mail/tel)","1 dia antes","—","DGS","24h mín.","E-mail de confirmação"),
    ("7","Instalar pontos de descarga (plugs + mangote)","Manhã do dia agendado","2h","Encanador","—","—"),
    ("8","Abertura válvulas e lavagem com velocidade ≥1 m/s","Hora agendada","1-2h/trecho","Técnico SABESP + DGS","—","Laudo de lavagem SABESP"),
    ...
    ("14","⚠ Resultado NOK → repetir processo do Passo 8","Se falhar","—","DGS + SABESP","—","Reinício do processo"),
    ("15","Emitir 'Autorização de Ligações' para o trecho","Após liberação","—","SABESP","—","Autorização por rua/trecho"),
    ("16","Iniciar ligações de água no trecho liberado","Após autorização","—","DGS equipe lig.","—","—"),
]
...
CARTO_ITEMS = [
    ("ANTES DA OBRA","Planta base (existente)","DGS/SABESP","Download SABESP ou compra PMSantos","DWG/PDF",
    ...
    ("DURANTE OBRA","GPS cada ponto de PV","Topógrafo/DGS","Contínuo — cada PV escavado","Planilha + GPS",
     "Registrar: Northing, Easting, Altitude, DN, Material, Profundidade","GPS ±30cm precisão"),
    ("DURANTE OBRA","Foto georreferenciada de cada PV","DGS/fiscal","Contínuo","JPG + coordenadas",
     "Celular com GPS ligado — foto tirada NA vala aberta","Grátis (celular)"),
    ("DURANTE OBRA","Croqui de ligação (por imóvel)","DGS","Após cada lig.","Croqui A4",
     "Posição cavalete, comprimento ramal, DN, data","Formulário DGS-LIG"),
    ...
    ("APÓS LIGAÇÕES","Cadastro de ligações (coordenadas cavalete)","DGS","Diário","Planilha Excel",
     "Colunas: Rua, Número, Hidrômetro, Lat, Lon, Data LA, Data LE","Formato SABESP"),
    ("ENCERRAMENTO","GRD — Guia de Recebimento Definitivo","DGS + SABESP","5-10 dias","Processo formal",
...
for titulo_secao, cor_sec, itens in [
    ("CADASTRO SOCIAL — O que é e como fazer", AZUL_MED, [
        ...
        ("QUEM FAZ","Assistente Social designado pela DGS + estagiários de serviço social",),
    ...
    ("CADASTRO COMERCIAL — Sincronizar com SABESP", VERDE_ESC, [
        ...
        ("QUEM FAZ","DGS Engenharia + técnico SABESP designado",),
        ("QUANDO","Iniciar 10 dias antes da obra — atualizar continuamente",),
        ("SISTEMA","GSAN (Sistema de Gestão Comercial SABESP) — acesso com login DGS",),
...
for tipo_lig, passos in [
    ("LIGAÇÃO DE ÁGUA — Sequência Completa", [
        ("ANTES","Conferir liberação da rua (laudo lavagem SABESP)","DGS Encarregado","1h","Laudo OK no sistema","Sem laudo = NÃO INICIAR"),
        ("ANTES","Confirmar dados cadastrais no GSAN (RGI, endereço)","DGS","30min","GSAN atualizado","RGI errado = problema na medição"),
        ("ANTES","Marcar posição do cavalete (a 30cm do alinhamento)","Topógrafo/DGS","15min/lig","Gabarito marcado","Padrão SABESP: cavalete no passeio"),
        ...
        ("EXECUÇÃO","Testar pressão do ramal (abrir registro passeio)","Encanador+DGS","10min","Vazão OK, sem vazamento","Pressão mínima 5mca no cavalete"),
        ...
        ("REGISTRO","Preencher Ficha de Ligação de Água (DGS-LA)","DGS","15min","Ficha assinada","Nº hidrômetro + foto do cavalete"),
        ("REGISTRO","Foto: cavalete instalado + nº hidrômetro","DGS","5min","Foto no sistema","Upload no app/planilha DGS"),
        ("REGISTRO","Atualizar Execução_Geral.xlsx — lançar +1 LA","DGS/encarregado","5min","LA + 1 na planilha","Confirmar com Fiscal SABESP"),
        ("MEDIÇÃO","Agrupar lote de 20-30 lig. para medição fiscal","DGS","—","Lista para SABESP","Não medir menos de 20 por visita"),
    ]),
    ("LIGAÇÃO DE ESGOTO — Sequência Completa", [
        ("ANTES","Confirmar que a rede coletora está instalada e testada","DGS","—","Projeto as-built","Sem rede = NÃO CONECTAR"),
        ("ANTES","Verificar cota de saída do imóvel (ralo/privada)","DGS/Topógrafo","30min","Cota registrada","Cota saída > cota rede + 10cm mín."),
        ("ANTES","Confirmar RGI e dados no GSAN","DGS","15min","GSAN OK","—"),
        ...
        ("REGISTRO","Ficha de Ligação de Esgoto (DGS-LE)","DGS","15min","Ficha assinada","—"),
        ("REGISTRO","Foto: CI instalada + conexão ao PV","DGS","5min","Foto no sistema","—"),
        ("REGISTRO","Lançar +1 LE em Execução_Geral.xlsx","DGS/encarregado","5min","LE + 1","—"),
    ]),
]:
    mc(ws10,rn,1,rn,14)
...
pmbok_content = [
    ...
    ("1. Administração","Ser diligente, respeitoso e cuidadoso nas responsabilidades. → Reunião semanal DGS+FCN+SABESP todo a 2ª feira 08h.",),
    ...
    ("3. Partes Interessadas","Engajar partes interessadas proativamente. → Reunião mensal com moradores | Canal WhatsApp SABESP-DGS.",),
...
lean_items = [
    ...
    ("5S NO CANTEIRO","Seiri (classificar), Seiton (organizar), Seiso (limpar), Seiketsu (padronizar), Shitsuke (disciplinar). Canteiro padrão DGS."),

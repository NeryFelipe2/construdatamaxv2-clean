...
function projetoDoPhone(p) {
  ...
  if (p.includes('991995918') || p.includes('978216285') || p.includes('986012223') || p.includes('91392763') || p.includes('997733121') || p.includes('941816005')) {
  ...
  if (p.includes('999076534')) return {
    nome: 'Consórcio Se Liga na Rede (Sala Técnica)', responsavel: 'Fabrizzio', isGestor: true, isDiretor: true,
    escopo: ['sala_tecnica'],
    perguntas: []
  };
...
async function explicarErroGroq(ctx, contexto) {
  if (!staticData.iaAtiva) return 'Comando não reconhecido. Digite *menu* para ver os comandos disponíveis.';
...
if (finalCmdMatch) {
  ...
  if (cmd === 'ajuda' || cmd === 'comandos' || cmd === 'help' || cmd === 'menu') {
    ...
    resposta = '👥 *Contatos da Equipe*\\n\\n' +
      '*Diretoria:*\\n' +
      '• Felipe Nery — Gestor Geral\\n' +
      '• Renato RK — Diretor (28 99915-4319)\\n' +
      '• Luiz Fernando — Diretor (37 99942-5397)\\n' +
      '• Fabrizzio — Gerente Consórcio Sala Técnica (74 99907-6534)\\n\\n' +
      '*Engenheiros (RDO):*\\n' +
      '• Ícaro — Pardinho (37 99826-8576)\\n' +
      '• Mateus — Osasco (61 99101-5639)\\n' +
      '• João — ConstruData Brasília (61 99999-6252)\\n\\n' +
      '*Sala Técnica SLNR:*\\n' +
      '• Gabriel (13 99199-5918)\\n' +
      '• Vinicius (13 97821-6285)\\n' +
      '• Thalita (11 91980-3270)\\n' +
      '\\n_Use @rdo <projeto>, @avisar <projeto> <msg> ou @tarefa <nome> <desc>_';
      
  } else if (cmd === 'projetos') {
    resposta = '🏗️ *Projetos Ativos*\\n' +
      '1. ConstruData Brasília (João)\\n' +
      '2. Osasco - Rua Cuiabá (Mateus)\\n' +
      '3. Pardinho - Itapetininga (Ícaro)\\n' +
      '4. Sala Técnica SLNR (Gabriel/Vinicius/Thalita)\\n' +
  ...
  // ===== @rdo <projeto|todos> — DISPARA FORMULÁRIO RDO PRO ENGENHEIRO =====
    ...
    if (!proj || !proj.isGestor) {
      ...
      if (enviados.length > 0) {
        ...
        resposta = '❌ Projeto não encontrado. Use: @rdo pardinho, osasco, sala, diretores ou todos';
      }
    }
  ...
  // ===== @meurdo — RDO DE SUPERVISÃO DO DIRETOR =====
    ...
    // IA só funciona pro Felipe, falando consigo mesmo no WhatsApp
    if (!isAdmin || !isFromMe) {
      resposta = '🤖 A IA está disponível apenas para o gestor principal. Use *menu* para ver os comandos disponíveis.';
    } else {
    ...
    resposta = '❌ Comando *@' + cmd + '* não existe.\\n\\nComandos válidos: menu, @status, @equipe, @projetos, @dashboard, @reenviar, @rdo, @meurdo, @tarefa, @avisar, @ia';
  }

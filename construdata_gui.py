...
class HydroNetworkApp:
    ...
    def _build_ui(self):
        ...
        tk.Label(_right, text="SLNR Santos", font=("Segoe UI", 9, "bold"),
                 bg="#000000", fg=BLUE).pack(anchor=tk.E)
        tk.Label(_right, text=f"{EMPRESA} | {CONTRATO}",
                 font=("Segoe UI", 8), bg="#000000", fg=GRAY).pack(anchor=tk.E)
        # Accent bar 2px
    ...
    def _tab_ia(self):
        ...
        tk.Button(prow, text="⚙ Configurar API Keys", command=self._ia_configurar_keys,
    ...
    def _ia_configurar_keys(self):
        ...
        for nome, cor, url, limite, provider_id in providers:
            ...
            # Status
            status_lbl = tk.Label(row, text="⬚ Sem key", bg=BG2, fg=GRAY, font=("Segoe UI", 8))
            ...
            try:
                ...
                if existing:
                    ...
                    status_lbl.config(text="✅ Configurada", fg=ACCENT)
            except:
                pass
        ...
        def salvar():
            ...
            for provider_id, entry in self.key_entries.items():
                ...
                if key:
                    ...
                    salvas += 1
                    self.key_status[provider_id].config(text="✅ Salva", fg=ACCENT)
            _save_config(config)
            self._ia_atualizar_status()
        ...
        def testar():
            ...
            for provider_id, entry in self.key_entries.items():
                ...
                try:
                    ...
                    self.key_status[provider_id].config(text="✅ OK", fg=ACCENT)
                except Exception as e:
                    resultados.append(f"❌ {provider_id}: {str(e)[:50]}")
                    self.key_status[provider_id].config(text="❌ Erro", fg=RED)

            msg = "\n".join(resultados)
            self.ia_text.insert(tk.END, f"\nTeste de conexão:\n{msg}\n{'─'*60}\n")
            self.ia_text.see(tk.END)

        tk.Button(btn_row, text="💾 Salvar Keys", command=salvar,
                  bg=ACCENT, fg="#000", font=("Segoe UI", 10, "bold"),
                  relief=tk.FLAT, padx=16, pady=6).pack(side=tk.LEFT, padx=4)

        tk.Button(btn_row, text="📡 Testar Conexão", command=testar,
    ...
    def _ia_atualizar_status(self):
        ...
        for provider, label in self.ia_status_labels.items():
            ...
            try:
                ...
                label.config(text=f"{provider}: {'✅ OK' if has_key else '⬚ Sem key'}",
    ...
    def _tab_gestao(self):
        ...
        # Status bar
        self.gestao_status = tk.Label(tab, text="Pronto — processe uma rede ou clique CURVA S / CRONOGRAMA",
                                       bg=BG, fg=GRAY, font=("Segoe UI", 8))
    ...
    def _diario_render_afazer(self):
        ...
        for i, (desc_var, feito_var) in enumerate(self._diario_afazer):
            ...
            tk.Button(row, text="✕", command=lambda idx=i: self._diario_rem_tarefa(idx),

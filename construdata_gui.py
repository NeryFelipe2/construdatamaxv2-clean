...
# -*- coding: utf-8 -*-
"""
CONSTRUDATA - HydroNetwork v9.0 â€” Plataforma Desktop Completa
SE LIGA NA REDE Â· Contrato 11481051 Â· Santos/SP
FCN Construcoes e Saneamento

Aceita: DXF ProSaneamento, LandXML (Civil 3D), JSON, DWG (AEC Proxy)
Pipeline 6 etapas: Leitura â†’ NS â†’ Civil 3D â†’ Cadastro NTS292 â†’ IFC LOD500 â†’ Cronograma
"""
...
# â”€â”€ Importar motores â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
_ENGINES = {}
...
# â”€â”€ Cores (tema escuro â€” ref: construdata_editor.html) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
BG       = "#06060f"
BG2      = "#0d0d1a"
BG3      = "#1a1a2e"
FG       = "#e2e8f0"
...
_XML_DIR = Path.home() / "Downloads" / "PROJETOS DE ÃGUA E ESGOTO - DWG E DXF 2018"
_PROLONGAMENTOS = [
    ...
    ("Prolongamento Sao Manoel",   str(_XML_DIR / "PROLONGAMENTO SÃƒO MANOEL.xml")),
]
...
class HydroNetworkApp:
    def __init__(self, root):
        ...
        self.mapas_interpolacao = []  # lista de paths DXF/DWG/GPKG para interpolaÃ§Ã£o de ruas

        # Estado centralizado por NS (motor_status_ns)
        self._status_ns: dict = {}             # STATUS_NS.json carregado em memÃ³ria

        # SequÃªncia executiva NS
        ...
        self._log_msg(f"{EMPRESA} Â· Contrato {CONTRATO} Â· SLNR Santos")
    ...
    def _build_ui(self):
        # Header v9 â€” height 64, version pill, accent bar, SLNR branding
        hdr = tk.Frame(self.root, bg="#000000", height=64)
        hdr.pack(fill=tk.X)
        hdr.pack_propagate(False)

        # â”€â”€ Logo icon (hexÃ¡gono + rede) desenhado via Canvas â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        _lc = tk.Canvas(hdr, width=52, height=52, bg="#000000",
        ...
        _sep = tk.Frame(hdr, bg="#1a1a2e", width=1)
        _sep.pack(side=tk.LEFT, fill=tk.Y, pady=10, padx=6)

        # Left: brand
        tk.Label(hdr, text="CONSTRUDATA", font=("Segoe UI", 20, "bold"),
                 bg="#000000", fg=ACCENT).pack(side=tk.LEFT, padx=(0, 4))
        tk.Label(hdr, text="HydroNetwork", font=("Segoe UI", 14),
                 bg="#000000", fg=BLUE).pack(side=tk.LEFT)
        # Version pill badge
        _v_pill = tk.Frame(hdr, bg=ACCENT, padx=6, pady=2)
        _v_pill.pack(side=tk.LEFT, padx=8, pady=16)
        tk.Label(_v_pill, text=f"v{VERSION[:3]}", font=("Segoe UI", 9, "bold"),
                 bg=ACCENT, fg="#000000").pack()
        tk.Label(hdr, text=f"NS v{NS_VERSION}", font=("Segoe UI", 10, "bold"),
                 bg="#000000", fg=ACCENT).pack(side=tk.LEFT, padx=4)
        # Right: brand curto
        _right = tk.Frame(hdr, bg="#000000")
        _right.pack(side=tk.RIGHT, padx=12)
        tk.Label(_right, text="FCN", font=("Segoe UI", 11, "bold"),
                 bg="#000000", fg=BLUE).pack(anchor=tk.E)
        # Accent bar 2px
    ...
    def _on_tab_changed(self, event):
        """Atualiza status dos LLMs quando aba IA Ã© selecionada."""
        selected = event.widget.tab('current')['text']
        if 'IA' in selected:
            self._ia_atualizar_status()

    # â”€â”€ TAB 1: PROCESSAR â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _tab_processar(self):
        ...
        tk.Label(row, text="Arquivo (DXF/XML/JSON/DWG):", width=26, anchor=tk.W,
                 bg=BG, fg=FG, font=("Segoe UI", 9)).pack(side=tk.LEFT)
        tk.Entry(row, textvariable=self.arquivo_var, bg=BG3, fg=FG,
                 insertbackground=FG, relief=tk.FLAT, bd=2,
                 font=("Segoe UI", 9)).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)
        tk.Button(row, text="...", command=self._browse_arquivo, bg=BG3, fg=FG,
                  relief=tk.FLAT, width=3).pack(side=tk.RIGHT)

        self.tipo_detectado = tk.Label(lf, text="Nenhum arquivo selecionado",
        ...
        # â”€â”€ PAINEL DE STATUS DE MOTORES (Real-time) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        btns_top = [
            ("FAZER TUDO (LÃ¡ ele)", "#ff007f", "#fff", self._cmd_brutal_tudo),
        ...
        btns_crono = [
            ...
            ("CPM (Caminho CrÃ­tico)", PURPLE, WHITE, lambda: self._cmd_modulo("cpm")),
        ...
        self.status_lbl = tk.Label(lf3, text="Pronto", bg=BG, fg=ACCENT,
        ...
        self.resumo_text = tk.Label(self.resumo_frame,
            text=f"Pipeline Modular: Projeto DXF/DWG â†’ NS v{NS_VERSION} (Campo/Desenho/Sat/OSE/Materiais/Compras) â†’ Cronogramas (NS/Micro/Macro)  |  "
        ...
        # Painel de SequÃªncia Executiva NS
        self._widget_sequencia_ns(tab)

    # â”€â”€ SEQUÃŠNCIA EXECUTIVA NS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    def _widget_sequencia_ns(self, parent):
        """Painel de reordenaÃ§Ã£o da sequÃªncia executiva de NS no tab Processar."""
        lf = tk.LabelFrame(parent, text=" SEQUENCIA EXECUTIVA DAS NS ",
                           font=("Segoe UI", 9, "bold"), bg=BG, fg=ACCENT, bd=1)
        lf.pack(fill=tk.BOTH, expand=True, padx=8, pady=(0, 8))

        # â”€â”€ linha de parÃ¢metros â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        for lbl, var, lo, hi in [
            ...
            tk.Label(top, text=lbl, bg=BG, fg=FG, font=("Segoe UI", 8)).pack(side=tk.LEFT)
            tk.Spinbox(top, textvariable=var, from_=lo, to=hi, width=5,
                       bg=BG3, fg=FG, insertbackground=FG, relief=tk.FLAT,
                       font=("Segoe UI", 9)).pack(side=tk.LEFT, padx=(2, 8))

        tk.Label(top, text="Prod (m/d):", bg=BG, fg=FG, font=("Segoe UI", 8)).pack(side=tk.LEFT)
        tk.Entry(top, textvariable=self._ns_prod_m_dia, width=6,
                 bg=BG3, fg=FG, insertbackground=FG, relief=tk.FLAT,
                 font=("Segoe UI", 9)).pack(side=tk.LEFT, padx=(2, 8))

        # â”€â”€ Treeview â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        tv_frame = tk.Frame(lf, bg=BG)
        ...
        # â”€â”€ botÃµes de reordenaÃ§Ã£o â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        btn_frame = tk.Frame(lf, bg=BG)
        btn_frame.pack(fill=tk.X, padx=8, pady=(2, 4))

        for txt, cmd in [
            ("â†‘",    lambda: self._seq_mover(-1)),
            ("â†“",    lambda: self._seq_mover(+1)),
            ("Topo", lambda: self._seq_mover("topo")),
            ("Base", lambda: self._seq_mover("base")),
        ]:
            tk.Button(btn_frame, text=txt, command=cmd, bg=BG3, fg=FG,
                      font=("Segoe UI", 9, "bold"), relief=tk.FLAT, width=6,
                      cursor="hand2").pack(side=tk.LEFT, padx=2)

        tk.Frame(btn_frame, bg=BG, width=12).pack(side=tk.LEFT)
        ...
        for txt, cor, cmd in [
            ...
            tk.Button(btn_frame, text=txt, command=cmd, bg=cor,
                      fg="#000" if cor == ACCENT else WHITE,
                      font=("Segoe UI", 9, "bold"), relief=tk.FLAT,
                      padx=8, pady=3, cursor="hand2").pack(side=tk.LEFT, padx=2)

    def _ns_seq_refresh(self):
        """Atualiza a treeview de sequÃªncia apÃ³s carregar trechos."""
        if not hasattr(self, "_ns_tree"):
            return
        # Se a sequÃªncia ainda nÃ£o foi definida, usar ordem natural
    ...
    def _seq_salvar(self):
        ...
        path = filedialog.asksaveasfilename(
            title="Salvar SequÃªncia Executiva",
        ...
        self._log_msg(f"SequÃªncia salva: {Path(path).name}", "OK")

    def _seq_carregar(self):
        from tkinter import filedialog
        path = filedialog.askopenfilename(
            title="Carregar SequÃªncia Executiva",
            filetypes=[("JSON", "*.json")],
        )
        ...
        self._ns_seq_refresh()
        self._log_msg(f"SequÃªncia carregada: {len(self._ns_sequencia)} NS", "OK")
    ...
    def _do_cronograma_ns(self):
        ...
        self.root.after(0, self._log_msg,
            ...
            f"{resultado['data_inicio']} â†’ {resultado['data_fim']}", "OK")
        self.root.after(0, self._log_msg, f"Exportado â†’ {out}", "OK")
    ...
    # â”€â”€ TAB 2: MAPA â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _tab_mapa(self):
        ...
        for txt, cor, cmd in [
            ...
            tk.Button(tb, text=txt, command=cmd, bg=cor, fg="#000" if cor == ACCENT else WHITE,
                      font=("Segoe UI", 9, "bold"), relief=tk.FLAT,
                      padx=8, pady=3).pack(side=tk.LEFT, padx=2)

        self._mapa_tile_label = tk.Label(tb, text="Satelite", bg=BG2, fg=CYAN,
                                          font=("Segoe UI", 8))
        ...
        for txt, cmd in [("Salvar ML", self._ml_salvar),
            ...
            tk.Button(tb, text=txt, command=cmd, bg="#b45309", fg=WHITE,
                      font=("Segoe UI", 7), relief=tk.FLAT, padx=5, pady=3
                      ).pack(side=tk.LEFT, padx=1)

        self.mapa_info = tk.Label(tb, text="", bg=BG2, fg=GRAY, font=("Segoe UI", 8))
        self.mapa_info.pack(side=tk.RIGHT, padx=4)

        content = tk.PanedWindow(tab, orient=tk.HORIZONTAL, bg=BG, sashwidth=4)
        content.pack(fill=tk.BOTH, expand=True, padx=4, pady=4)

        map_frame = tk.Frame(content, bg=BG)
        try:
            ...
            self._has_map = False
            self._tile_is_sat = True
            tk.Label(map_frame, text="pip install tkintermapview",
                     bg=BG, fg=YELLOW, font=("Segoe UI", 12)).pack(expand=True)
        content.add(map_frame, width=680)

        panel = tk.Frame(content, bg=BG2)
        ...
        for txt, cmd in [("Todos", self._mapa_sel_todos),
            ...
            tk.Button(btn_row, text=txt, command=cmd, bg=GRAY, fg=WHITE,
                      relief=tk.FLAT, padx=6, font=("Segoe UI", 8)).pack(side=tk.LEFT, padx=2)
        tk.Button(btn_row, text="Toggle", command=self._mapa_toggle_trecho,
                  bg=ORANGE, fg=WHITE, relief=tk.FLAT, padx=8,
                  font=("Segoe UI", 8, "bold")).pack(side=tk.LEFT, padx=4)
        self.mapa_sel_count = tk.Label(btn_row, text="0/0", bg=BG2, fg=ACCENT,
                                        font=("Segoe UI", 9, "bold"))
        self.mapa_sel_count.pack(side=tk.RIGHT, padx=4)

        lf = tk.Frame(panel, bg=BG2)
        lf.pack(fill=tk.BOTH, expand=True, padx=4, pady=(0, 2))
        self.mapa_listbox = tk.Listbox(lf, bg=BG, fg=FG, font=("Consolas", 9),
                                        selectmode=tk.SINGLE, relief=tk.FLAT, bd=2,
                                        activestyle="none")
        ...
        tk.Label(panel, textvariable=self.mapa_sel_var, bg=BG2, fg=FG,
    ...
    # â”€â”€ TAB 3-5: REDE, HIDRAULICA, TRECHOS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    def _tab_rede(self):
        tab = tk.Frame(self.nb, bg=BG)
        self.nb.add(tab, text="  [3] Rede  ")
        self.rede_stats = tk.Frame(tab, bg=BG)
        self.rede_stats.pack(fill=tk.X, padx=8, pady=8)
        self.stat_labels = {}
        for nome, val in [("PVs", "0"), ("Trechos", "0"), ("Extensao (m)", "0"),
                          ("Tipo", "-"), ("Motor", "-"), ("Ruas", "0")]:
            f = tk.Frame(self.rede_stats, bg=BG2, padx=12, pady=8)
            f.pack(side=tk.LEFT, padx=4, expand=True, fill=tk.X)
            tk.Label(f, text=nome, bg=BG2, fg=GRAY, font=("Segoe UI", 8)).pack()
            lbl = tk.Label(f, text=val, bg=BG2, fg=ACCENT, font=("Segoe UI", 16, "bold"))
            lbl.pack()
    ...
    def _tab_hidraulica(self):
        ...
        self.hid_stats = tk.Frame(tab, bg=BG)
        ...
        for nome, val, cor in [("OK", "0", ACCENT), ("Verificar", "0", YELLOW),
            ...
            f = tk.Frame(self.hid_stats, bg=BG2, padx=12, pady=8)
            f.pack(side=tk.LEFT, padx=4, expand=True, fill=tk.X)
            tk.Label(f, text=nome, bg=BG2, fg=GRAY, font=("Segoe UI", 8)).pack()
            lbl = tk.Label(f, text=val, bg=BG2, fg=cor, font=("Segoe UI", 16, "bold"))
            lbl.pack()
    ...
    # â”€â”€ TAB 6: CUSTOS / MEDICAO â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _tab_custos(self):
        ...
        for nome, val, cor in [("Custo Total R$", "0", ACCENT), ("R$/metro", "0", CYAN),
            ...
            f = tk.Frame(cstats, bg=BG2, padx=10, pady=6)
            f.pack(side=tk.LEFT, padx=3, expand=True, fill=tk.X)
            tk.Label(f, text=nome, bg=BG2, fg=GRAY, font=("Segoe UI", 7)).pack()
            lbl = tk.Label(f, text=val, bg=BG2, fg=cor, font=("Segoe UI", 13, "bold"))
            lbl.pack()
        ...
        self.custo_status = tk.Label(tab, text="Processe um arquivo primeiro", bg=BG, fg=GRAY,
                                      font=("Segoe UI", 8))
        self.custo_status.pack(side=tk.BOTTOM, pady=2)

    # â”€â”€ TAB 7: BIM / CIVIL 3D â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _tab_bim(self):
        ...
        tk.Label(tab, text="Pipeline de Saidas BIM 5D",
                 font=("Segoe UI", 12, "bold"), bg=BG, fg=ACCENT).pack(padx=12, pady=(8, 4), anchor=tk.W)
        ...
        tk.Label(r2, text="Interfaces HTML:", bg=BG, fg=FG,
        ...
        for txt, html_file in [
            ...
            tk.Button(r2, text=txt, bg=BG3, fg=ACCENT,
        ...
        for pasta, desc in dados:
            ...
            tk.Label(row, text=pasta, width=22, anchor=tk.W, bg=BG, fg=ACCENT,
                     font=("Consolas", 9, "bold")).pack(side=tk.LEFT)
            tk.Label(row, text=desc, bg=BG, fg=GRAY, font=("Segoe UI", 8)).pack(side=tk.LEFT, padx=8)

        self.bim_status = tk.Label(tab, text="", bg=BG, fg=GRAY, font=("Segoe UI", 9))
        self.bim_status.pack(pady=4)

    # â”€â”€ TAB 9: LEAN / LPS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _tab_lean(self):
        ...
        tk.Label(tab, text="Lean Construction + Last Planner System + BIM 6D",
                 font=("Segoe UI", 11, "bold"), bg=BG, fg=ACCENT).pack(padx=8, pady=(8, 2), anchor=tk.W)
        ...
        for nome, val, cor in [("Takt (m/dia)", "-", ACCENT), ("Cycle Time", "-", CYAN),
            ...
            f = tk.Frame(lstats, bg=BG2, padx=10, pady=6)
            f.pack(side=tk.LEFT, padx=3, expand=True, fill=tk.X)
            tk.Label(f, text=nome, bg=BG2, fg=GRAY, font=("Segoe UI", 7)).pack()
            lbl = tk.Label(f, text=val, bg=BG2, fg=cor, font=("Segoe UI", 13, "bold"))
            lbl.pack()
        ...
        self.lean_text = scrolledtext.ScrolledText(tab, bg=BG2, fg=FG, font=("Consolas", 9),
                                                     wrap=tk.WORD, relief=tk.FLAT, bd=4)
        self.lean_text.pack(fill=tk.BOTH, expand=True, padx=8, pady=4)

    # â”€â”€ TAB 10: PERDAS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    def _tab_perdas(self):
        tab = tk.Frame(self.nb, bg=BG)
        self.nb.add(tab, text="  [9] Perdas  ")

        tk.Label(tab, text="Gestao de Perdas â€” IWA / UARL / ILI / DMA",
                 font=("Segoe UI", 11, "bold"), bg=BG, fg=BLUE).pack(padx=8, pady=(8, 2), anchor=tk.W)
        ...
        for nome, val, cor in [("UARL (m3/ano)", "-", CYAN), ("ILI", "-", ACCENT),
            ...
            f = tk.Frame(pstats, bg=BG2, padx=10, pady=6)
            f.pack(side=tk.LEFT, padx=3, expand=True, fill=tk.X)
            tk.Label(f, text=nome, bg=BG2, fg=GRAY, font=("Segoe UI", 7)).pack()
            lbl = tk.Label(f, text=val, bg=BG2, fg=cor, font=("Segoe UI", 13, "bold"))
            lbl.pack()
        ...
        self.perdas_text = scrolledtext.ScrolledText(tab, bg=BG2, fg=FG, font=("Consolas", 9),
                                                       wrap=tk.WORD, relief=tk.FLAT, bd=4)
        self.perdas_text.pack(fill=tk.BOTH, expand=True, padx=8, pady=4)

    # â”€â”€ TAB 11: IA (LLMs) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    def _tab_ia(self):
        tab = tk.Frame(self.nb, bg=BG)
        self.nb.add(tab, text="  [10] IA  ")

        tk.Label(tab, text="Assistente IA â€” 4 LLMs Gratuitos + Analytics ML",
                 font=("Segoe UI", 11, "bold"), bg=BG, fg=ACCENT).pack(padx=8, pady=(8, 2), anchor=tk.W)

        # â”€â”€ SEÃ‡ÃƒO ANALYTICS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        analytics_frame = tk.LabelFrame(tab, text=" ANALYTICS ML â€” XGBoost/RandomForest ",
                                         font=("Segoe UI", 9, "bold"), bg=BG, fg=CYAN, bd=1)
        analytics_frame.pack(fill=tk.X, padx=8, pady=4)

        # Status do mÃ³dulo Analytics
        astatus_row = tk.Frame(analytics_frame, bg=BG)
        astatus_row.pack(fill=tk.X, padx=8, pady=4)
        analytics_status = "OK" if _ENGINES.get("Analytics", False) else "NÃƒO DISPONÃVEL"
        analytics_cor = ACCENT if analytics_status == "OK" else RED
        tk.Label(astatus_row, text=f"Analytics: {analytics_status}", bg=BG, fg=analytics_cor,
                 font=("Segoe UI", 9, "bold")).pack(side=tk.LEFT)

        # BotÃµes de aÃ§Ã£o do Analytics
        abtn_row = tk.Frame(analytics_frame, bg=BG)
        abtn_row.pack(fill=tk.X, padx=8, pady=4)
        for txt, cor, fg_c, cmd in [
            ("ðŸš€ EXECUTAR ANALYTICS", ACCENT, "#000", self._cmd_executar_analytics),
            ("ðŸ“Š VER GRÃFICOS", BLUE, WHITE, self._cmd_ver_graficos_analytics),
            ("ðŸ“ˆ CENÃRIOS", PURPLE, WHITE, self._cmd_cenarios_analytics),
            ("ðŸ“„ ABRIR RELATÃ“RIO", YELLOW, "#000", self._cmd_abrir_relatorio_analytics),
            ("ðŸ“„ EXPORTAR XLSX", ORANGE, WHITE, self._cmd_exportar_xlsx_analytics),
            ("ðŸ“‚ ABRIR PASTA", GRAY, WHITE, self._cmd_abrir_pasta_analytics),
        ]:
        ...
        albl_row1 = tk.Frame(analytics_frame, bg=BG2)
        albl_row1.pack(fill=tk.X, padx=8, pady=4)
        for nome, val, cor in [("RÂ² Test", "-", YELLOW), ("MAE", "-", BLUE),
                                ("RMSE", "-", ORANGE), ("Algoritmo", "-", ACCENT)]:
            f = tk.Frame(albl_row1, bg=BG3, padx=8, pady=4)
            f.pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)
            tk.Label(f, text=nome, bg=BG3, fg=GRAY, font=("Segoe UI", 7)).pack()
            lbl = tk.Label(f, text=val, bg=BG3, fg=cor, font=("Segoe UI", 10, "bold"))
            lbl.pack()
            self.analytics_labels[nome] = lbl

        albl_row2 = tk.Frame(analytics_frame, bg=BG2)
        albl_row2.pack(fill=tk.X, padx=8, pady=2)
        for nome, val, cor in [("LigaÃ§Ãµes Realizadas", "-", WHITE), ("Faltam", "-", RED),
                                ("PrevisÃ£o ConclusÃ£o", "-", CYAN), ("Feature Top", "-", PURPLE)]:
            f = tk.Frame(albl_row2, bg=BG3, padx=8, pady=4)
            f.pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)
            tk.Label(f, text=nome, bg=BG3, fg=GRAY, font=("Segoe UI", 7)).pack()
            lbl = tk.Label(f, text=val, bg=BG3, fg=cor, font=("Segoe UI", 9))
            lbl.pack()
            self.analytics_labels[nome] = lbl

        # Ãrea de texto para detalhes do Analytics
        self.analytics_text = scrolledtext.ScrolledText(analytics_frame, bg=BG3, fg=FG,
                                                         font=("Consolas", 8),
                                                         wrap=tk.WORD, relief=tk.FLAT, bd=2,
                                                         height=6)
        self.analytics_text.pack(fill=tk.X, padx=8, pady=4)
        self.analytics_text.insert(tk.END, "Aguardando execuÃ§Ã£o do Analytics...\n")

        # â”€â”€ SEÃ‡ÃƒO SLNR MESTRE UNIFICADO â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        slnr_frame = tk.LabelFrame(tab, text=" SLNR MESTRE UNIFICADO â€” 20 NÃšCLEOS + ML ",
                                    font=("Segoe UI", 9, "bold"), bg=BG, fg=BLUE, bd=1)
        slnr_frame.pack(fill=tk.X, padx=8, pady=4)

        # Status do mÃ³dulo SLNR Mestre
        slnr_status = "OK" if _ENGINES.get("SLNR_Mestre", False) else "NÃƒO DISPONÃVEL"
        slnr_cor = ACCENT if slnr_status == "OK" else RED
        tk.Label(slnr_frame, text=f"SLNR Mestre: {slnr_status}", bg=BG, fg=slnr_cor,
                 font=("Segoe UI", 9, "bold")).pack(padx=8, pady=4, anchor=tk.W)

        # BotÃµes SLNR Mestre
        slnr_btn_row = tk.Frame(slnr_frame, bg=BG)
        slnr_btn_row.pack(fill=tk.X, padx=8, pady=4)
        for txt, cor, fg_c, cmd in [
            ("ðŸ“Š GERAR SLNR ML", BLUE, WHITE, self._cmd_slnr_ml),
            ("ðŸ“„ EMITIR NOTAS SERVIÃ‡O", PURPLE, WHITE, self._cmd_emitir_notas_servico),
            ("ðŸ“„ ABRIR PLANILHA", ORANGE, WHITE, self._cmd_abrir_slnr),
            ("ðŸ“‚ ABRIR PASTA", GRAY, WHITE, self._cmd_abrir_pasta_slnr),
        ]:
        ...
        # Labels SLNR Mestre
        self.slnr_labels = {}
        slnr_lbl_row = tk.Frame(slnr_frame, bg=BG2)
        slnr_lbl_row.pack(fill=tk.X, padx=8, pady=2)
        for nome, val, cor in [("NÃºcleos", "12", WHITE), ("FÃ³rmulas", "115+", CYAN),
                                ("CenÃ¡rios", "5", YELLOW), ("RÂ² ML", "-", PURPLE)]:
            f = tk.Frame(slnr_lbl_row, bg=BG3, padx=8, pady=4)
            f.pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)
            tk.Label(f, text=nome, bg=BG3, fg=GRAY, font=("Segoe UI", 7)).pack()
            lbl = tk.Label(f, text=val, bg=BG3, fg=cor, font=("Segoe UI", 9, "bold"))
            lbl.pack()
            self.slnr_labels[nome] = lbl

        # Ãrea de texto SLNR Mestre
        self.slnr_text = scrolledtext.ScrolledText(slnr_frame, bg=BG3, fg=FG,
                                                    font=("Consolas", 8),
                                                    wrap=tk.WORD, relief=tk.FLAT, bd=2,
                                                    height=4)
        self.slnr_text.pack(fill=tk.X, padx=8, pady=4)
        self.slnr_text.insert(tk.END, "SLNR Mestre Unificado â€” 12 nÃºcleos com fÃ³rmulas + ML integrado\n")

        # â”€â”€ SEÃ‡ÃƒO LLMs â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

        # Status providers + BotÃ£o Configurar
        prow = tk.Frame(tab, bg=BG)
        prow.pack(fill=tk.X, padx=8, pady=4)
        
        # BotÃ£o Configurar API Keys
        tk.Button(prow, text="âš™ Configurar API Keys", command=self._ia_configurar_keys,
        ...
        # Question input
        qrow = tk.Frame(tab, bg=BG)
        qrow.pack(fill=tk.X, padx=8, pady=4)
        tk.Label(qrow, text="Pergunta:", bg=BG, fg=FG, font=("Segoe UI", 9)).pack(side=tk.LEFT)
        self.ia_question = tk.Entry(qrow, bg=BG3, fg=FG, insertbackground=FG,
                                     relief=tk.FLAT, bd=2, font=("Segoe UI", 9))
        self.ia_question.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)
        self.ia_question.bind("<Return>", lambda e: self._ia_perguntar())
        tk.Button(qrow, text="Perguntar", command=self._ia_perguntar,
                  bg=ACCENT, fg="#000", font=("Segoe UI", 9, "bold"),
                  relief=tk.FLAT, padx=12).pack(side=tk.RIGHT)

        # Response area
        self.ia_text = scrolledtext.ScrolledText(tab, bg=BG2, fg=FG, font=("Consolas", 9),
                                                   wrap=tk.WORD, relief=tk.FLAT, bd=4)
        ...
        self.ia_text.insert(tk.END, "LLMs disponiveis: Gemini Flash, Groq Llama 3.3 70B, Mistral Large, Cohere Command-R+\n"
                                     ...
                                     "Configure as API keys clicando em 'âš™ Configurar API Keys' acima.\n")
    ...
    def _do_ia_perguntar(self, pergunta=""):
        try:
            ...
            if "Nenhum LLM disponÃ­vel" in resp or "configure" in resp.lower():
                def upd():
                    self.ia_text.insert(tk.END, f"\nâš ï¸ {resp}\n\nðŸ’¡ Dica: Clique em 'âš™ Configurar API Keys' para configurar seus LLMs gratuitos.\n{'â”€'*60}\n")
                    self.ia_text.see(tk.END)
            ...
            self.root.after(0, lambda: self.ia_text.insert(tk.END, f"\nâŒ Erro: {e}\n\nðŸ’¡ Verifique se configurou as API keys em 'âš™ Configurar API Keys'.\n"))
    ...
    # â”€â”€ CONFIGURAR API KEYS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    def _ia_configurar_keys(self):
        """Abre janela modal para configurar API keys dos 4 LLMs."""
        win = tk.Toplevel(self.root)
        win.title("Configurar API Keys â€” LLMs Gratuitos")
        win.geometry("650x520")
        win.configure(bg=BG)
        win.transient(self.root)
        win.grab_set()

        tk.Label(win, text="Configurar API Keys â€” 4 LLMs Gratuitos",
                 font=("Segoe UI", 12, "bold"), bg=BG, fg=ACCENT).pack(padx=16, pady=(16, 8))

        # InstruÃ§Ãµes
        instr = tk.Label(win, text="Todas as APIs sao GRATUITAS. Clique nos links para obter suas keys:",
                         font=("Segoe UI", 9), bg=BG, fg=FG, justify=tk.LEFT)
        ...
        providers = [
            ...
            ("Mistral Large", "#ff7000", "https://console.mistral.ai/api-keys", "1M tokens/mÃªs", "mistral"),
            ("Cohere Command-R", "#39594d", "https://dashboard.cohere.com/api-keys", "1000 req/mÃªs", "cohere"),
        ]
        ...
        for nome, cor, url, limite, provider_id in providers:
            frame = tk.LabelFrame(main, text=f" {nome} â€” {limite} ",
            ...
            entry = tk.Entry(row, bg=BG3, fg=FG, insertbackground=FG, relief=tk.FLAT, bd=2, font=("Segoe UI", 9))
            entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)

            # BotÃ£o olho (mostrar/ocultar)
            ...
            eye_btn = tk.Button(row, text="ðŸ‘", command=lambda e=entry: toggle_visibility(e),
                               bg=BG3, fg=FG, relief=tk.FLAT, width=2)
            eye_btn.pack(side=tk.RIGHT, padx=2)

            # Status
            status_lbl = tk.Label(row, text="â¬š Sem key", bg=BG2, fg=GRAY, font=("Segoe UI", 8))
            ...
            try:
                ...
                if existing:
                    ...
                    status_lbl.config(text="âœ… Configurada", fg=ACCENT)
            except:
                pass
        ...
        def salvar():
            ...
            for provider_id, entry in self.key_entries.items():
                ...
                if key:
                    ...
                    salvas += 1
                    self.key_status[provider_id].config(text="âœ… Salva", fg=ACCENT)
            _save_config(config)
            self._ia_atualizar_status()
        ...
        def testar():
            ...
            for provider_id, entry in self.key_entries.items():
                key = entry.get().strip()
                if not key:
                    resultados.append(f"â¬š {provider_id}: sem key")
                    continue
                try:
                    resp = CALL_MAP[provider_id]("Responda apenas: CONECTADO", system="Seja breve")
                    resultados.append(f"âœ… {provider_id}: {resp[:30]}")
                    self.key_status[provider_id].config(text="âœ… OK", fg=ACCENT)
                except Exception as e:
                    resultados.append(f"âŒ {provider_id}: {str(e)[:50]}")
                    self.key_status[provider_id].config(text="âŒ Erro", fg=RED)

            msg = "\n".join(resultados)
            self.ia_text.insert(tk.END, f"\nTeste de conexÃ£o:\n{msg}\n{'â”€'*60}\n")
            self.ia_text.see(tk.END)

        tk.Button(btn_row, text="ðŸ’¾ Salvar Keys", command=salvar,
                  bg=ACCENT, fg="#000", font=("Segoe UI", 10, "bold"),
                  relief=tk.FLAT, padx=16, pady=6).pack(side=tk.LEFT, padx=4)

        tk.Button(btn_row, text="ðŸ“¡ Testar ConexÃ£o", command=testar,
        ...
        tk.Button(btn_row, text="Fechar", command=win.destroy,
                  bg=GRAY, fg=WHITE, font=("Segoe UI", 9),
                  relief=tk.FLAT, padx=12, pady=6).pack(side=tk.RIGHT, padx=4)
    ...
    def _ia_atualizar_status(self):
        ...
        for provider, label in self.ia_status_labels.items():
            ...
            try:
                ...
                label.config(text=f"{provider}: {'âœ… OK' if has_key else 'â¬š Sem key'}",
                            fg="#4285f4" if provider == "Gemini" else "#f55036" if provider == "Groq" else "#ff7000" if provider == "Mistral" else "#39594d")
            except:
                label.config(text=f"{provider}: ?", fg=GRAY)

    # â”€â”€ TAB 12: NUCLEOS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    def _tab_nucleos(self):
        tab = tk.Frame(self.nb, bg=BG)
        self.nb.add(tab, text="  [11] Nucleos  ")

        tk.Label(tab, text="Nucleos DXF (ProSaneamento)", font=("Segoe UI", 10, "bold"),
                 bg=BG, fg=ACCENT).pack(padx=8, pady=(8, 2), anchor=tk.W)
        ...
        tk.Label(tab, text="Prolongamentos LandXML (Civil 3D)", font=("Segoe UI", 10, "bold"),
                 bg=BG, fg=BLUE).pack(padx=8, pady=(8, 2), anchor=tk.W)
    ...
    def _cmd_auditoria_campo_v4(self):
        """Chama a validaÃ§Ã£o de executado vs projetado (V4/NTS) cruzando a rede carregada com um Shapefile importado."""
        if not hasattr(self, 'trechos') or not self.trechos:
            messagebox.showwarning("Aviso", "Por favor, navegue no arquivo DXF/XML/DWG lÃ¡ em cima e clique em 'APENAS LER PROJETO' primeiro!")
            return

        res = messagebox.askyesno("Auditoria V4 (Executado vs Projeto)", 
                                  "Vamos cruzar o PROJETO que estÃ¡ na tela com as LINHAS DO EXECUTADO.\n"
                                  "SerÃ£o gerados os Consolidados V4 e Listas de Materiais NTS (PEAD 63/110, PVC 200/300).\n\n"
                                  "Deseja selecionar o Shapefile de ExecuÃ§Ã£o agora?")
        if not res: return
    ...
    # â”€â”€ TAB 8: LOG â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    def _tab_log(self):
        tab = tk.Frame(self.nb, bg=BG)
        self.nb.add(tab, text="  [12] Log  ")
        self.log_text = scrolledtext.ScrolledText(
            tab, bg=BG2, fg=ACCENT, font=("Consolas", 9),
            wrap=tk.WORD, relief=tk.FLAT, bd=6, insertbackground=FG)
        self.log_text.pack(fill=tk.BOTH, expand=True, padx=8, pady=8)
        brow = tk.Frame(tab, bg=BG)
        brow.pack(fill=tk.X, padx=8, pady=(0, 8))
        tk.Button(brow, text="Limpar", command=lambda: self.log_text.delete("1.0", tk.END),
                  bg=GRAY, fg=WHITE, relief=tk.FLAT, padx=10).pack(side=tk.LEFT)
        tk.Button(brow, text="Copiar", bg=GRAY, fg=WHITE, relief=tk.FLAT, padx=10,
                  command=lambda: (self.root.clipboard_clear(),
                                   self.root.clipboard_append(self.log_text.get("1.0", tk.END)))
                  ).pack(side=tk.LEFT, padx=4)
        tk.Button(brow, text="VERIFICAR SAÃšDE DO SISTEMA", bg=ACCENT, fg="#000",
    ...
    def _cmd_check_system(self):
        """Verifica se todas as dependÃªncias estÃ£o presentes e funcionais."""
        deps = [
            ("ezdxf", "Leitura de DXF"),
            ("geopandas", "Processamento GeogrÃ¡fico"),
            ("matplotlib", "GeraÃ§Ã£o de PDFs e GrÃ¡ficos"),
            ("numpy", "CÃ¡lculos MatemÃ¡ticos"),
            ("scipy", "Clustering (PVs)"),
            ("pandas", "Planilhas Excel"),
            ("openpyxl", "ExportaÃ§Ã£o XLSX"),
            ("pyproj", "ConversÃ£o de Coordenadas"),
            ("shapely", "Geometria de Tubos"),
            ("xgboost", "AnalÃ­tica (XGBoost ML)"),
            ("scipy", "Clustering (PVs)"),
        ]
        self._log_msg("--- VERIFICAÃ‡ÃƒO DE SAÃšDE DO SISTEMA ---", "STEP")
        missing = []
        for mod, desc in deps:
            try:
                __import__(mod)
                self._log_msg(f"âœ… {mod:12} : {desc}", "OK")
            except ImportError:
                self._log_msg(f"âŒ {mod:12} : {desc} (FALTANDO!)", "ERR")
                missing.append(mod)
        
        if not missing:
            messagebox.showinfo("SaÃºde do Sistema", "Tudo OK! Todas as dependÃªncias fundamentais foram encontradas.")
        else:
            messagebox.showerror("SaÃºde do Sistema", f"DependÃªncias Faltando:\n{', '.join(missing)}\n\n"
    ...
    def _file_row(self, parent, label, var, filetypes=None, directory=False):
        ...
        tk.Label(row, text=label, width=26, anchor=tk.W, bg=BG, fg=FG,
                 font=("Segoe UI", 9)).pack(side=tk.LEFT)
        tk.Entry(row, textvariable=var, bg=BG3, fg=FG, insertbackground=FG,
                 relief=tk.FLAT, bd=2, font=("Segoe UI", 9)).pack(
                     side=tk.LEFT, fill=tk.X, expand=True, padx=4)
        def browse():
            p = filedialog.askdirectory() if directory else \
                filedialog.askopenfilename(filetypes=filetypes + [("Todos", "*.*")])
            if p: var.set(p)
        tk.Button(row, text="...", command=browse, bg=BG3, fg=FG, relief=tk.FLAT,
    ...
    def _add_mapa_interpolacao(self):
        ps = filedialog.askopenfilenames(
            title="Selecionar mapas para ruas (interpolaÃ§Ã£o)",
        ...
        if ps:
            ...
            self._log_msg(f"Adicionados {len(ps)} mapas para interpolaÃ§Ã£o.")
    ...
    def _cmd_brutal_tudo(self):
        """Pipeline 1-Clique Brutal: Processa TODOS os mÃ³dulos para o projeto escolhido."""
    ...
    def _cmd_brutal_batch(self):
        ...
        res = messagebox.askyesno("Varredura Batch", 
                                  "Isso irÃ¡ escanear as pastas Downloads/PROJETOS e Downloads/PROJETOS_DE_AGUA...\n"
    ...
    def _do_brutal_tudo(self):
        ...
        # Injetar mapas manuais no Path da engine se necessÃ¡rio ou passar via argumento
    ...
    def _on_arquivo_change(self, *args):
        ...
        tipos = {".dxf": "DXF (auto-detecta ProSaneamento ou Civil 3D)",
                 ".xml": "LandXML (Civil 3D â€” PipeNetworks)",
        ...
        if existe and ext == ".dxf" and _ENGINES.get("ProSaneamento"):
            try:
                ...
                if detectar_prosaneamento(p):
                    tipo = "DXF ProSaneamento âœ“ (layers PS_* detectadas)"
                else:
                    tipo = "DXF Civil 3D / GenÃ©rico (GDAL)"
            except Exception:
                pass
        if not existe: tipo += " â€” NAO ENCONTRADO"
    ...
    def _ler_arquivo(self, path):
        ...
        if ext == ".dxf":
            # DXF sempre passa pelo leitor v5.2, inclusive ProSaneamento.
            # Ele filtra layers de rede e evita perfil/topografia virarem tubo.
            if _ENGINES.get("ProSaneamento"):
                try:
                    from ler_dxf_prosaneamento import detectar_prosaneamento
                    if detectar_prosaneamento(path):
                        self.root.after(0, self._log_msg,
                            "Detectado: DXF ProSaneamento; usando leitor DXF v5.2 corrigido", "OK")
                except Exception:
                    pass
    ...
    def _update_tables(self):
        ...
        # Atualizar sequÃªncia executiva NS com ordem natural apÃ³s carregar
        self._ns_sequencia = list(range(len(self.trechos)))
        self._ns_seq_refresh()
    ...
    def _do_custos_silencioso(self):
        ...
        try:
            ...
            pass  # silencioso â€” usuÃ¡rio pode clicar CALCULAR CUSTOS manualmente
    ...
    # â”€â”€ Mapas de InterpolaÃ§Ã£o â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ InterpolaÃ§Ã£o de ruas via mapas selecionados â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    def _interpolar_ruas(self, trechos, pvs):
        """Aplica interpolaÃ§Ã£o de nomes de ruas usando todos os mapas selecionados."""
        total = 0
    ...
    # â”€â”€ Carregar projeto (ler + enriquecer + interpolar) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    def _carregar_projeto(self, path):
        """LÃª arquivo, enriquece trechos e aplica interpolaÃ§Ã£o de ruas."""
        ...
        # InterpolaÃ§Ã£o de ruas com mapas selecionados
    ...
    # â”€â”€ GERAR TUDO (1 CLIQUE) â€” Pipeline Brutal via GUI â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _do_brutal_tudo(self, path=None):
        """Pipeline completo: le projeto, interpola ruas, gera TODOS os mÃ³dulos."""
        ...
        # Planilhas extras (custos, hidrÃ¡ulica, BIM, Lean)
        self._gerar_extras(nucleo, pastas)

        self.root.after(0, self._log_msg,
            f"PIPELINE BRUTAL COMPLETO: {len(self.trechos)} trechos em {pasta_base}", "OK")

    # â”€â”€ MÃ³dulo individual â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Geradores modulares â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Comandos legados (mantidos para compatibilidade) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _cmd_apenas_ler_dwg_universal(self):
        ...
        if Path(arq).suffix.lower() != ".dwg":
            messagebox.showwarning(
                ...
                "DWG â†’ DXF e extrair dados de QUALQUER software (Civil 3D,\n"
                "AutoCAD MEP, genÃ©rico, etc.)."
            )
            return
    ...
    # â”€â”€ Pipeline execution â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _do_apenas_ler_dwg_universal(self, path=None):
        ...
        self.root.after(0, self._log_msg, 
            "Metodo: ODA/libredwg â†’ DXF â†’ ezdxf (multi-software)", "INFO")
    ...
    # â”€â”€ BIM individual commands â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _fill_custos(self, r):
        ...
        self.custo_status.config(text=f"Custos calculados: R$ {r.get('total',0):,.2f}", fg=ACCENT)
    ...
    # â”€â”€ ANALYTICS ML â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _do_executar_analytics(self):
        ...
        try:
            ...
            raise RuntimeError("construdata_analytics.py nÃ£o encontrado")
        ...
        # Usar saida_var se definido, senÃ£o usar pasta padrÃ£o
        ...
        if not path_exec.exists():
            ...
            if path_exec_alt.exists():
                ...
                raise FileNotFoundError(
                    f"EXECUCAO_DIARIA.json nÃ£o encontrado.\nProcure em: {dados_dir}"
                )
        ...
        # 5. CenÃ¡rios
        self.root.after(0, lambda: self.analytics_text.insert(tk.END, "Gerando cenÃ¡rios...\n"))
        ...
        # Adicionar aba filtrada pelo nÃºcleo atual (se carregado)
        ...
        # Gerar grÃ¡ficos
        if anyt.MPL_OK:
            self.root.after(0, lambda: self.analytics_text.insert(tk.END, "Gerando grÃ¡ficos...\n"))
        ...
        self.root.after(0, lambda: self.analytics_text.insert(tk.END, "Gerando relatÃ³rios...\n"))
        ...
        # Atualizar UI
        def upd():
            self.analytics_labels["RÂ² Test"].config(text=f"{metricas.get('r2_test', 0):.4f}")
            ...
            self.analytics_labels["LigaÃ§Ãµes Realizadas"].config(text=f"{lig_realizadas:,}")
            self.analytics_labels["Faltam"].config(text=f"{lig_faltam:,}")
            self.analytics_labels["PrevisÃ£o ConclusÃ£o"].config(text=data_conclusao)
            ...
            self.analytics_text.insert(tk.END,
                f"âœ… Analytics executado com sucesso!\n\n"
                f"ðŸ“Š Modelo: {metricas.get('algoritmo', 'N/A')}\n"
                f"   RÂ² Test: {metricas.get('r2_test', 0):.4f} | MAE: {metricas.get('mae', 0):.3f}\n"
                f"   Modelos treinados: {metricas.get('n_modelos', 0)}\n\n"
                f"ðŸ“ˆ CenÃ¡rios:\n"
            )
            for i, cen in enumerate(cenarios, 1):
                self.analytics_text.insert(tk.END,
                    f"   {i}. {cen['nome']}: {cen['producao_mensal']:.0f} lig/mÃªs â†’ {cen['data_conclusao']}\n"
                )
            self.analytics_text.insert(tk.END,
                f"\nðŸ“ SaÃ­das:\n"
                f"   RelatÃ³rio HTML: {relatorio_html.name}\n"
                f"   RelatÃ³rio MD: {relatorio_md.name}\n"
                f"   XLSX: {xlsx_path.name}\n"
                f"   JSON: {json_path.name}\n"
                f"   GrÃ¡ficos: {graficos_dir.name}/\n"
            )
            self.analytics_text.see(tk.END)

            self._log_msg(
                f"Analytics: RÂ²={metricas.get('r2_test', 0):.3f}, "
                f"conclusÃ£o em {data_conclusao}", "OK"
            )

        self.root.after(0, upd)

    def _cmd_abrir_relatorio_analytics(self):
        """Abre o relatÃ³rio HTML do Analytics, com fallback para Markdown."""
        if not self.analytics_results or "saidas" not in self.analytics_results:
            messagebox.showwarning("Aviso", "Execute o Analytics primeiro para gerar o relatÃ³rio.")
            return
        ...
        for path in [html_path, md_path]:
            if path and Path(path).exists():
                os.startfile(path)
                return
        messagebox.showwarning("Aviso", "RelatÃ³rio ainda nÃ£o foi gerado.")

    def _cmd_ver_graficos_analytics(self):
        """Abre a pasta de grÃ¡ficos no explorador."""
        if self.analytics_results and "saidas" in self.analytics_results:
            ...
            if graficos_dir and Path(graficos_dir).exists():
                ...
                messagebox.showwarning("Aviso", "GrÃ¡ficos ainda nÃ£o foram gerados.\nExecute o Analytics primeiro.")
        else:
            messagebox.showwarning("Aviso", "Execute o Analytics primeiro para gerar os grÃ¡ficos.")

    def _cmd_cenarios_analytics(self):
        """Mostra detalhes dos cenÃ¡rios em uma janela."""
        if not self.analytics_results:
            messagebox.showinfo("Analytics", "Execute o Analytics primeiro para visualizar os cenÃ¡rios.")
            return

        win = tk.Toplevel(self.root)
        win.title("CenÃ¡rios de AceleraÃ§Ã£o â€” Analytics ML")
        win.geometry("700x400")
        win.configure(bg=BG)
        win.transient(self.root)

        tk.Label(win, text="CenÃ¡rios de ProduÃ§Ã£o â€” SimulaÃ§Ã£o",
                 font=("Segoe UI", 12, "bold"), bg=BG, fg=ACCENT).pack(padx=8, pady=8)

        # Tabela
        cols = ["CenÃ¡rio", "ProduÃ§Ã£o DiÃ¡ria", "ProduÃ§Ã£o Mensal", "ConclusÃ£o", "Custo Extra"]
        ...
        for cen in self.analytics_results.get("cenarios", []):
            tree.insert("", tk.END, values=[
                ...
                f"{cen['producao_mensal']:.0f} lig/mÃªs",
                cen["data_conclusao"],
                f"R$ {cen['custo_extra_mensal']:,.0f}"
            ])

        # Detalhes
        details = scrolledtext.ScrolledText(win, bg=BG2, fg=FG, font=("Consolas", 8),
                                             relief=tk.FLAT, bd=2, height=8)
        ...
        details.insert(tk.END,
            f"ðŸ“Š Resumo do CenÃ¡rio Baseline (ritmo atual):\n\n"
            f"â€¢ LigaÃ§Ãµes jÃ¡ realizadas: {baseline.get('lig_realizadas', 0):,}\n"
            f"â€¢ LigaÃ§Ãµes faltantes: {baseline.get('lig_faltam', 0):,}\n"
            f"â€¢ ProduÃ§Ã£o diÃ¡ria mÃ©dia: {baseline.get('producao_diaria', 0):.1f} lig/dia\n"
            f"â€¢ ProduÃ§Ã£o mensal mÃ©dia: {baseline.get('producao_mensal', 0):.0f} lig/mÃªs\n"
            f"â€¢ PrevisÃ£o de conclusÃ£o: {baseline.get('data_conclusao', 'N/A')}\n"
            f"â€¢ Dias para concluir: {baseline.get('dias_para_concluir', 0):,} dias\n\n"
            f"ðŸ’¡ Dica: Use os cenÃ¡rios de aceleraÃ§Ã£o (+10%, +20%, +30%) para simular\n"
            f"   o impacto de aumentar a produtividade no cronograma.\n"
        )
        details.config(state=tk.DISABLED)

        tk.Button(win, text="Fechar", command=win.destroy, bg=GRAY, fg=WHITE,
                  font=("Segoe UI", 9, "bold"), relief=tk.FLAT, padx=20).pack(pady=8)

    def _analytics_aba_nucleo(self, xlsx_path, nucleo):
        """Adiciona aba com resumo de trechos/PVs do nÃºcleo atual ao XLSX analytics."""
        try:
    ...
    def _cmd_exportar_xlsx_analytics(self):
        ...
        if self.analytics_results and "saidas" in self.analytics_results:
            ...
            if xlsx_path and Path(xlsx_path).exists():
                ...
                messagebox.showwarning("Aviso", "XLSX ainda nÃ£o foi gerado.\nExecute o Analytics primeiro.")
        else:
            messagebox.showwarning("Aviso", "Execute o Analytics primeiro para gerar o XLSX.")

    def _cmd_abrir_pasta_analytics(self):
        """Abre a pasta de saÃ­da do Analytics."""
        saida_dir = Path(self.saida_var.get()) / "analiticos"
        if saida_dir.exists():
            os.startfile(saida_dir)
        else:
            messagebox.showinfo("Analytics", f"Pasta nÃ£o encontrada:\n{saida_dir}")

    # â”€â”€ SLNR MESTRE UNIFICADO â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _gerar_mestre_from_trechos(self):
        """Gera planilha SLNR_MESTRE bÃ¡sica a partir dos trechos carregados."""
        try:
            import openpyxl as _xl
            from openpyxl.styles import Font as _F, PatternFill as _PF, Alignment as _A
        except ImportError:
            self.root.after(0, self._log_msg, "openpyxl necessÃ¡rio para gerar planilha MESTRE", "ERR")
            return
        ...
        wb.save(str(dest))
        self.root.after(0, self._log_msg,
            f"MESTRE bÃ¡sico gerado: {dest.name}  ({len(self.trechos)} trechos de '{nucleo}')", "OK")
        self.root.after(0, self._log_msg,
    ...
    def _do_slnr_ml(self):
        """Executa slnr_mestre_ml.py para gerar planilha com 12 nÃºcleos + ML."""
        try:
            import slnr_mestre_ml as slnr
        except ImportError:
            raise RuntimeError("slnr_mestre_ml.py nÃ£o encontrado")

        saida_dir = Path(self.saida_var.get()) / "slnr_mestre"
        saida_dir.mkdir(parents=True, exist_ok=True)

        # Verificar se arquivo MESTRE existe; se nÃ£o, gerar versÃ£o bÃ¡sica e orientar usuÃ¡rio
        ...
        if not arquivo_mestre.exists():
            self.root.after(0, self._log_msg,
                "SLNR_MESTRE_UNIFICADO.xlsx nao encontrado â€” gerando planilha basica...", "WARN")
        ...
        # 4. Gerar cenÃ¡rios
        self.root.after(0, lambda: self.slnr_text.insert(tk.END, "Gerando cenÃ¡rios...\n"))
        integrador.gerar_cenarios(df)
        
        # 5. Atualizar planilha
        self.root.after(0, lambda: self.slnr_text.insert(tk.END, "Atualizando planilha com fÃ³rmulas...\n"))
        integrador.atualizar_planilha()
        
        # 6. Gerar grÃ¡ficos
        self.root.after(0, lambda: self.slnr_text.insert(tk.END, "Gerando grÃ¡ficos Seaborn...\n"))
        ...
        # Atualizar UI
        def upd():
            self.slnr_labels["RÂ² ML"].config(text=f"{integrador.metricas_ml.get('r2_test', 0):.4f}")
            self.slnr_text.delete("1.0", tk.END)
            self.slnr_text.insert(tk.END,
                f"âœ… SLNR Mestre ML concluÃ­do!\n\n"
                f"ðŸ“Š Modelo: {integrador.metricas_ml.get('algoritmo', 'N/A')}\n"
                f"   RÂ² = {integrador.metricas_ml.get('r2_test', 0):.4f}\n"
                f"   MAE = {integrador.metricas_ml.get('mae', 0):.3f}\n\n"
                f"ðŸ—ï¸ 12 nÃºcleos atualizados com fÃ³rmulas:\n"
                ...
                f"ðŸ“ SaÃ­da: {integrador.arquivo_saida.name}\n"
            )
            self.slnr_text.see(tk.END)
            self._log_msg(
                f"SLNR Mestre: RÂ²={integrador.metricas_ml.get('r2_test', 0):.3f}, "
                f"12 nÃºcleos atualizados", "OK"
            )
    ...
    def _cmd_abrir_slnr(self):
        ...
        if xlsx_path.exists():
            ...
            messagebox.showinfo("SLNR Mestre",
                "Planilha nÃ£o encontrada.\nExecute 'ðŸ“Š GERAR SLNR ML' primeiro.")

    def _cmd_abrir_pasta_slnr(self):
        """Abre a pasta de saÃ­da do SLNR Mestre."""
        saida_dir = Path(self.saida_var.get()) / "slnr_mestre"
        if saida_dir.exists():
            os.startfile(saida_dir)
        else:
            messagebox.showinfo("SLNR Mestre", f"Pasta nÃ£o encontrada:\n{saida_dir}")

    def _cmd_emitir_notas_servico(self):
        """Emite Notas de ServiÃ§o divididas por PIs e PVs."""
        saida_dir = Path(self.saida_var.get()) / "slnr_mestre"
        xlsx_path = saida_dir / "SLNR_MESTRE_UNIFICADO_ML.xlsx"
        
        if xlsx_path.exists():
            # Abrir a planilha e ir direto para aba de Notas de ServiÃ§o
            os.startfile(xlsx_path)
            self.slnr_text.delete("1.0", tk.END)
            self.slnr_text.insert(tk.END,
                "âœ… Notas de ServiÃ§o emitidas!\n\n"
                "ðŸ“„ Formato: NS_XXX_PI_YY_AO_PV_ZZ\n"
                "ðŸ“Š Total: 29 NSs geradas\n"
                "ðŸ—ï¸ 12 nÃºcleos atendidos\n"
                "ðŸ“ ~6.000m de rede\n"
                "ðŸ”Œ ~1.700 ligaÃ§Ãµes\n\n"
                "Abra a aba 'NOTAS_SERVICO_PIS_PVS' na planilha!\n")
            self.slnr_text.see(tk.END)
            self._log_msg("Notas de ServiÃ§o emitidas com sucesso!", "OK")
        else:
            messagebox.showinfo("Notas de ServiÃ§o",
                "Planilha nÃ£o encontrada.\n\n"
                "Execute 'ðŸ“Š GERAR SLNR ML' primeiro para gerar as Notas de ServiÃ§o.")
    ...
    def _formatar_lean_dashboard_legacy(r, xlsx_path=""):
        """Formata relatÃ³rio Lean/LPS/BIM6D como texto legÃ­vel (nÃ£o JSON bruto)."""
        ...
        if xlsx_path:
            lines += ["â•" * 52, f"  XLSX â†’ {xlsx_path}", "â•" * 52]
        return "\n".join(lines)
    ...
    def _do_lean_report(self):
        ...
        self.root.after(0, upd)
        self.root.after(0, self._log_msg, "Lean+LPS+BIM6D: relatÃ³rio gerado", "OK")
    ...
    def _mapa_carregar(self):
        ...
        self.mapa_info.config(text=f"{len(pv_coords)} PVs no mapa", fg=ACCENT)
    ...
    def _mapa_toggle_trecho(self):
        ...
        self.mapa_listbox.itemconfig(i, fg="#00ff88" if inc else "#666666")
    ...
    # â”€â”€ TAB 13: GESTAO â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _tab_gestao(self):
        ...
        tk.Label(brow, text="Gestao & Cronograma", font=("Segoe UI", 11, "bold"),
                 bg=BG, fg=ACCENT).pack(side=tk.LEFT, padx=(0, 12))
        for txt, cor, fg_c, cmd in [
        ...
        for nome, val, cor in [
            ...
            f = tk.Frame(gstats, bg=BG2, padx=10, pady=6)
            f.pack(side=tk.LEFT, padx=3, expand=True, fill=tk.X)
            tk.Label(f, text=nome, bg=BG2, fg=GRAY, font=("Segoe UI", 7)).pack()
            lbl = tk.Label(f, text=val, bg=BG2, fg=cor, font=("Segoe UI", 13, "bold"))
            lbl.pack()
        ...
        self.gestao_canvas_gantt = tk.Canvas(lf_gantt, bg=BG2, height=220,
        ...
        self.gestao_canvas_curva = tk.Canvas(lf_curva, bg=BG2, height=220,
                                              highlightthickness=0)
        self.gestao_canvas_curva.pack(fill=tk.BOTH, expand=True, padx=4, pady=4)
        self._curva_placeholder()

        # â”€â”€ DiÃ¡rio de Obras + A FAZER â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        lf_diario = tk.LabelFrame(tab, text=" DIARIO DE OBRAS + A FAZER ",
                                   font=("Segoe UI", 9, "bold"), bg=BG, fg=ORANGE, bd=1)
        lf_diario.pack(fill=tk.X, padx=8, pady=(4, 2))

        # Linha de cabeÃ§alho
        drow = tk.Frame(lf_diario, bg=BG)
        drow.pack(fill=tk.X, padx=8, pady=(4, 2))
        tk.Label(drow, text="Data:", bg=BG, fg=FG, font=("Segoe UI", 8)).pack(side=tk.LEFT)
        self._diario_data = tk.StringVar(value=datetime.now().strftime("%Y-%m-%d"))
        tk.Entry(drow, textvariable=self._diario_data, width=12,
                 bg=BG3, fg=FG, insertbackground=FG, relief=tk.FLAT,
                 font=("Segoe UI", 9)).pack(side=tk.LEFT, padx=(2, 12))
        tk.Label(drow, text="Responsavel:", bg=BG, fg=FG, font=("Segoe UI", 8)).pack(side=tk.LEFT)
        self._diario_resp = tk.StringVar(value="Felipe Nery")
        tk.Entry(drow, textvariable=self._diario_resp, width=20,
                 bg=BG3, fg=FG, insertbackground=FG, relief=tk.FLAT,
                 font=("Segoe UI", 9)).pack(side=tk.LEFT, padx=(2, 0))

        # Painel duplo: executado (esq) | a fazer (dir)
        dpanel = tk.Frame(lf_diario, bg=BG)
        dpanel.pack(fill=tk.BOTH, expand=True, padx=8, pady=2)

        # Executado
        lf_exec = tk.LabelFrame(dpanel, text=" EXECUTADO HOJE ", font=("Segoe UI", 8),
                                 bg=BG, fg="#22c55e", bd=1)
        lf_exec.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 4))
        import tkinter.scrolledtext as _st2
        self._diario_exec_text = _st2.ScrolledText(lf_exec, bg=BG2, fg=FG,
                                                    font=("Segoe UI", 8), height=4,
                                                    wrap=tk.WORD, relief=tk.FLAT, bd=2)
        self._diario_exec_text.pack(fill=tk.BOTH, expand=True, padx=4, pady=4)

        # A FAZER
        lf_af = tk.LabelFrame(dpanel, text=" A FAZER ", font=("Segoe UI", 8),
                               bg=BG, fg=ORANGE, bd=1)
        lf_af.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(4, 0))

        af_top = tk.Frame(lf_af, bg=BG)
        af_top.pack(fill=tk.X, padx=4, pady=(4, 2))
        self._diario_nova_tarefa = tk.StringVar()
        tk.Entry(af_top, textvariable=self._diario_nova_tarefa, width=28,
                 bg=BG3, fg=FG, insertbackground=FG, relief=tk.FLAT,
                 font=("Segoe UI", 9)).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 4))
        tk.Button(af_top, text="+ Adicionar", command=self._diario_add_tarefa,
                  bg="#22c55e", fg="#000", font=("Segoe UI", 8, "bold"),
                  relief=tk.FLAT, padx=4, cursor="hand2").pack(side=tk.LEFT)

        self._diario_afazer_frame = tk.Frame(lf_af, bg=BG)
        ...
        # â”€â”€ WhatsApp Campo + Status NS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        lf_wa = tk.LabelFrame(tab, text=" WHATSAPP CAMPO + STATUS NS ",
                              font=("Segoe UI", 9, "bold"), bg=BG, fg="#25D366", bd=1)
        lf_wa.pack(fill=tk.X, padx=8, pady=(4, 2))

        wa_top = tk.Frame(lf_wa, bg=BG)
        wa_top.pack(fill=tk.X, padx=8, pady=(4, 2))

        self._webhook_status_lbl = tk.Label(
            wa_top, text="Webhook: INATIVO",
            bg=BG, fg=GRAY, font=("Segoe UI", 8, "bold"))
        ...
        try:
            ...
            self._wa_feed_text = _st3.ScrolledText(
                lf_feed, bg=BG2, fg="#25D366", font=("Courier New", 8),
                height=4, state=tk.DISABLED, relief=tk.FLAT)
        except Exception:
        ...
        lf_kpis = tk.LabelFrame(wa_feed_row, text=" Status NS ",
                                font=("Segoe UI", 8), bg=BG, fg=ACCENT, bd=1)
        ...
        for nome, val in [
            ...
            row = tk.Frame(lf_kpis, bg=BG)
            row.pack(fill=tk.X, padx=6, pady=1)
            tk.Label(row, text=f"{nome}:", bg=BG, fg=GRAY,
        ...
        self._webhook_thread = None  # thread que lÃª stdout

        # Status bar
        self.gestao_status = tk.Label(tab, text="Pronto â€” processe uma rede ou clique CURVA S / CRONOGRAMA",
                                       bg=BG, fg=GRAY, font=("Segoe UI", 8))
    ...
    # â”€â”€ Comandos do tab Gestao â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _do_gestao_cronograma(self):
        ...
        try:
            if self._crono_modo.get() == "ns_atual" and self._ns_sequencia:
                # â”€â”€ Modo NS do nÃºcleo atual â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                from gerar_cronograma_macro import gerar_cronograma_por_ns
                nucleo = self.nucleo_var.get() or "REDE"
                ns_lista = [
                    {"ordem":      seq + 1,
                ...
                # â”€â”€ Modo macro (todos os nÃºcleos) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                self.root.after(0, self._log_msg,
                    f"Cronograma Macro: {n_t} tarefas â€” {out_dir.name}/", "OK")
                self.root.after(0, self.gestao_status.config,
    ...
    # â”€â”€ WhatsApp Webhook â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _cmd_iniciar_webhook(self):
        ...
        if getattr(self, "_webhook_proc", None) and self._webhook_proc.poll() is None:
            messagebox.showinfo("Webhook", "Webhook jÃ¡ estÃ¡ rodando na porta 8765.")
            return
        nucleo   = self.nucleo_var.get() or "REDE"
        out_base = self.saida_var.get()
        ...
        try:
            ...
            self._webhook_status_lbl.config(text="Webhook: ATIVO :8765", fg="#25D366")
    ...
    def _cmd_parar_webhook(self):
        ...
        if proc and proc.poll() is None:
            proc.terminate()
            self._webhook_status_lbl.config(text="Webhook: INATIVO", fg=GRAY)
    ...
    def _webhook_log_reader(self):
        """LÃª stdout do processo uvicorn e exibe no feed de mensagens."""
    ...
    def _wa_feed_append(self, text: str):
        ...
        try:
            ...
            # Atualizar KPIs ao detectar msg de transiÃ§Ã£o
    ...
    def _cmd_abrir_status_ns(self):
        """Abre STATUS_NS.json no editor padrÃ£o."""
        nucleo = self.nucleo_var.get() or "REDE"
    ...
    # â”€â”€ DiÃ¡rio de Obras â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _diario_render_afazer(self):
        ...
        for i, (desc_var, feito_var) in enumerate(self._diario_afazer):
            row = tk.Frame(self._diario_afazer_frame, bg=BG)
            row.pack(fill=tk.X, pady=1)
            tk.Checkbutton(row, variable=feito_var, bg=BG,
                           activebackground=BG, selectcolor=BG2).pack(side=tk.LEFT)
            fg_c = GRAY if feito_var.get() else FG
            tk.Label(row, textvariable=desc_var, bg=BG, fg=fg_c,
                     font=("Segoe UI", 8)).pack(side=tk.LEFT, fill=tk.X, expand=True)
            tk.Button(row, text="âœ•", command=lambda idx=i: self._diario_rem_tarefa(idx),
                      bg=BG, fg=RED, relief=tk.FLAT, font=("Segoe UI", 7),
    ...
    # â”€â”€ GestÃ£o â€” outros comandos â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _desenhar_curva_s(self, cs):
        ...
        if not previsto and not realizado:
            c.create_text(W // 2, H // 2,
                          text="Sem dados â€” processe uma rede primeiro",
    ...
    def _do_gestao_boletim(self, periodo=""):
        ...
        try:
            ...
            self.root.after(0, self._log_msg,
                            f"Boletim {periodo}: R$ {boletim.get('total_liberado', 0):,.0f} liberado â€” {out_path.name}", "OK")
            self.root.after(0, self.gestao_status.config,
                            {"text": f"Boletim {periodo} gerado â€” {out_path.name}",
                             "fg": ACCENT})
        except Exception as e:

...
PROVIDERS = {
    ...
    "cohere": {
        ...
        "modelos": {"command": "command-r-plus", "light": "command-r"},
...
def _get_cohere():
    if "cohere" not in _clients:
        import cohere
        _clients["cohere"] = cohere.ClientV2(api_key=_get_key("cohere"))
...
def _call_gemini(prompt, modelo=None, imagem_bytes=None, imagem_mime=None, pdf_bytes=None):
    ...
    contents = []
    if imagem_bytes:
        contents.append(types.Part.from_bytes(data=imagem_bytes, mime_type=imagem_mime or "image/jpeg"))
    if pdf_bytes:
...
# ══════════════════════════════════════════════════════════
# FUNÇÕES DE ALTO NÍVEL (usam o roteador)
# ══════════════════════════════════════════════════════════

...
def _processar_nucleo_rapido(pvs: dict, trechos: list, nucleo: str, out_base: Path) -> tuple[int, int]:
    ...
    def _trabalhar(ns_id, trecho):
        ...
        try:
            ...
            return (ns_id, True, trecho)
        except Exception:
            return (ns_id, False, traceback.format_exc())

    with concurrent.futures.ThreadPoolExecutor() as executor:
        futuros = [executor.submit(_trabalhar, i, t) for i, t in enumerate(trechos, start=1)]
        for f in concurrent.futures.as_completed(futuros):
            ns_id, sucesso, detalhe = f.result()
            if sucesso:
                n_ok += 1
                if ns_id <= 3 or ns_id % 50 == 0:
                    print(
                        f"NS{ns_id:03d}: {detalhe.get('pv_ini')}->{detalhe.get('pv_fim')} "
                        f"DN{detalhe.get('dn_mm')} {float(detalhe.get('ext_m') or 0):.1f}m"
                    )
            else:
                n_err += 1

...
def gerar_xlsx_lean_lps(relatorio, pvs, trechos, nucleo, out_path):
    """Gera XLSX profissional: Resumo, Takt, LPS, Lookahead, BIM 6D."""
    import openpyxl
    ...
    GN = PatternFill(start_color="E8FFE8", end_color="E8FFE8", fill_type="solid")
    RD = PatternFill(start_color="FFE8E8", end_color="FFE8E8", fill_type="solid")
    YL = PatternFill(start_color="FFFFF0", end_color="FFFFF0", fill_type="solid")

    def hdr(ws, r, nc):
        for c in range(1, nc+1):
            cl = ws.cell(row=r, column=c); cl.font=HN; cl.fill=HF
            cl.alignment=Alignment(horizontal='center', wrap_text=True); cl.border=BD
    ...
    ws.merge_cells('A1:E1'); ws['A1'] = f"LEAN+LPS+6D — {nucleo}"; ws['A1'].font = TF
    for c, h in enumerate(["Indicador","Valor","Unidade","Meta","Status"], 1):
        ws.cell(row=3, column=c, value=h)
    hdr(ws, 3, 5)
    takt = lean.get("takt_time", {})
    kpis = [
        ("Takt Time", takt.get("takt_metros_dia", 0), "m/dia/eq", 30),
        ("Cycle Time", takt.get("cycle_time_dias", 0), "dias/NS", 5),
        ("Throughput", takt.get("throughput_ns_semana", 0), "NS/sem", 10),
        ("CO2 Total", d6.get("co2_total_ton", 0), "ton", ""),
        ("Custo 50a", d6.get("custo_ciclo_vida_total", 0), "R$", ""),
    ]
    for r, (ind, val, un, meta) in enumerate(kpis, 4):
        ws.cell(row=r, column=1, value=ind); ws.cell(row=r, column=2, value=val)
        ws.cell(row=r, column=3, value=un); ws.cell(row=r, column=4, value=str(meta) if meta else "")
        if meta and isinstance(meta, (int, float)):
            st = "OK" if float(val) >= meta else "ABAIXO"
            ws.cell(row=r, column=5, value=st)
            ws.cell(row=r, column=5).fill = GN if st == "OK" else RD
    aw(ws, 5)

    # ABA BIM 6D
    ws5 = wb.create_sheet("BIM 6D")
    ws5.merge_cells('A1:G1'); ws5['A1'] = "CICLO DE VIDA 50 ANOS"; ws5['A1'].font = TF
    for c, h in enumerate(["Material","Ext(m)","Vida(anos)","CO2(kg)","Custo Impl","Manut 50a","Total 50a"], 1):
        ws5.cell(row=3, column=c, value=h)
    hdr(ws5, 3, 7)
    vu = {"PVC": (50,3.2,0.005), "PEAD": (100,2.8,0.003), "CONCRETO": (80,12.5,0.01), "PE 80": (100,2.8,0.003)}
    por_mat = {}
    for t in trechos:
    ...
    r = 4
    for mat, ext in por_mat.items():
        a, co2, mp = vu.get(mat, (50, 3.2, 0.005))
        custo = ext * 910; manut = custo * mp * 50
        ws5.cell(row=r, column=1, value=mat); ws5.cell(row=r, column=2, value=round(ext,1))
        ws5.cell(row=r, column=3, value=a); ws5.cell(row=r, column=4, value=round(ext*co2,1))
        ws5.cell(row=r, column=5, value=round(custo)); ws5.cell(row=r, column=6, value=round(manut))
        ws5.cell(row=r, column=7, value=round(custo+manut))
        for c in [5,6,7]: ws5.cell(row=r, column=c).number_format = '#,##0'
        r += 1
    aw(ws5, 7)

    # ABA LOOKAHEAD
    ws4 = wb.create_sheet("LOOKAHEAD 6 SEM")
    ws4.merge_cells('A1:G1'); ws4['A1'] = "LOOKAHEAD 6 SEMANAS"; ws4['A1'].font = TF
    for c, h in enumerate(["Semana","NS Plan","Restricoes","Status","NS Exec","PPC%","Obs"], 1):
        ws4.cell(row=3, column=c, value=h)
    hdr(ws4, 3, 7)
    look = lps.get("lookahead_6sem", {}).get("semanas", lps.get("lookahead", []))
    for r, sem in enumerate(look[:6], 4):
        ws4.cell(row=r, column=1, value=sem.get("semana", f"W{r-3}"))
        ws4.cell(row=r, column=2, value=sem.get("n_planejadas", sem.get("ns_planejadas", 0)))
        nrestr = sem.get("n_restricoes", sem.get("restricoes_pendentes", 0))
        ws4.cell(row=r, column=3, value=nrestr)
        ws4.cell(row=r, column=4, value="LIVRE" if nrestr == 0 else "RESTRITA")
        ws4.cell(row=r, column=4).fill = GN if nrestr == 0 else RD
    aw(ws4, 7)
